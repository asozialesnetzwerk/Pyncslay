...
from syncplay.vendor.Qt.QtWidgets import (
    QApplication,
    QLineEdit,
    QLabel,
    QCheckBox,
...
class GuiConfiguration:
    ...
    def run(self):
        ...
        dialog = ConfigDialog(
            self.config, self._availablePlayerPaths, self.error, self.defaultConfig
        )
...
class ConfigDialog(QtWidgets.QDialog):
    ...
    def moreToggled(self):
        ...
        if self.moreToggling is False:
            ...
            if self.showmoreCheckbox.isChecked():
                ...
                if isMacOS():
                    self.mediaplayerSettingsGroup.setFixedHeight(
                        self.mediaplayerSettingsGroup.minimumSizeHint().height()
                    )
                self.stackedFrame.setFixedHeight(
                    self.stackedFrame.minimumSizeHint().height()
                )
            else:
                ...
                if isMacOS():
                    ...
                    newHeight = (
                        self.connectionSettingsGroup.minimumSizeHint().height()
                        + self.mediaplayerSettingsGroup.minimumSizeHint().height()
                        + self.bottomButtonFrame.minimumSizeHint().height()
                        + 50
                    )
                else:
                    newHeight = (
    ...
    def _tryToFillPlayerPath(self, playerpath, playerpathlist):
        ...
        if playerpath is not None and playerpath != "":
            if utils.isURL(playerpath):
                ...
                if not playerPathExists(playerpath):
                    expandedpath = PlayerFactory().getExpandedPlayerPathByPath(
                        playerpath
                    )
                    if expandedpath is not None and playerPathExists(expandedpath):
                        playerpath = expandedpath
                    elif "mpvnet.exe" in playerpath and playerPathExists(
                        playerpath.replace("mpvnet.exe", "mpvnet.com")
                    ):
        ...
        for path in playerpathlist:
            ...
            elif playerPathExists(path) and os.path.normcase(
                os.path.normpath(path)
            ) != os.path.normcase(os.path.normpath(foundpath)):
                self.executablepathCombobox.addItem(path)
                if foundpath == "":
                    foundpath = path
    ...
    def _updateExecutableIcon(self, iconpath, playerpath):
        if iconpath is not None and iconpath != "":
            if iconpath.endswith(".mng"):
                ...
                self.executableiconLabel.setPixmap(
                    QtGui.QPixmap.fromImage(self.executableiconImage)
                )
        else:
    ...
    def updatePlayerArguments(self, currentplayerpath):
        argumentsForPath = utils.getPlayerArgumentsByPathAsText(
            self.perPlayerArgs, currentplayerpath
        )
    ...
    def changedPlayerArgs(self):
        ...
        if currentplayerpath:
            NewPlayerArgs = (
                utils.parseCommandLineString(self.playerargsTextbox.text())
                if self.playerargsTextbox.text()
                else ""
            )
    ...
    def languageChanged(self):
        setLanguage(
            str(self.languageCombobox.itemData(self.languageCombobox.currentIndex()))
        )
        QtWidgets.QMessageBox.information(
            self, "Syncplay", getMessage("language-changed-msgbox-label")
        )
        ...
        if fileName:
            if isMacOS() and fileName.endswith(".app"):  # see GitHub issue #91
                ...
                try:
                    ...
                    IndexError
                ):  # whoops, looks like this .app doesn't contain a executable file at all
                    pass
        ...
        try:
            ...
            if self.lastCheckedForUpdates:
                if (
                    ...
                    and self.config["lastCheckedForUpdates"] != ""
                ):
                    if self.lastCheckedForUpdates.toPython() > datetime.strptime(
                        self.config["lastCheckedForUpdates"], "%Y-%m-%d %H:%M:%S.%f"
                    ):
                        self.config["lastCheckedForUpdates"] = (
                            self.lastCheckedForUpdates.toString("yyyy-MM-d HH:mm:ss.z")
                        )
                else:
                    self.config["lastCheckedForUpdates"] = (
                        self.lastCheckedForUpdates.toString("yyyy-MM-d HH:mm:ss.z")
    ...
    def browseMediapath(self):
        ...
        options = QtWidgets.QFileDialog.Options()
        if IsPySide:
            if self.config["mediaSearchDirectories"] and os.path.isdir(
                self.config["mediaSearchDirectories"][0]
            ):
                ...
                defaultdirectory = QDesktopServices.storageLocation(
        ...
        browserfilter = "All files (*)"
        fileName, filtr = QtWidgets.QFileDialog.getOpenFileName(
            self, "Browse for media files", defaultdirectory, browserfilter, "", options
        )
        if fileName:
    ...
    def _saveDataAndLeave(self, storeConfiguration=True):
        ...
        self.config["mediaSearchDirectories"] = utils.convertMultilineStringToList(
            self.mediasearchTextEdit.toPlainText()
        )
        self.config["trustedDomains"] = utils.convertMultilineStringToList(
            self.trusteddomainsTextEdit.toPlainText()
        )
        ...
        if self.hostCombobox.currentText():
            self.config["host"] = (
                self.hostCombobox.currentText()
                if ":" in self.hostCombobox.currentText()
                else self.hostCombobox.currentText() + ":" + str(constants.DEFAULT_PORT)
            )
            self.config["host"] = (
                self.config["host"]
                .replace(" ", "")
        ...
        self.config["playerPath"] = str(
            self.safenormcaseandpath(self.executablepathCombobox.currentText())
        )
        self.config["language"] = str(
            self.languageCombobox.itemData(self.languageCombobox.currentIndex())
        )
        if self.mediapathTextbox.text() == "":
    ...
    def processWidget(self, container, torun):
        for widget in container.children():
            ...
            if (
                ...
                and widget.objectName()[:3] != "qt_"
            ):
                torun(widget)

    def loadTooltips(self, widget):
        tooltipName = (
            widget.objectName().lower().split(constants.CONFIG_NAME_MARKER)[0]
            + "-tooltip"
        )
        if (
            tooltipName[:1] == constants.INVERTED_STATE_MARKER
    ...
    def loadValues(self, widget):
        ...
        if isinstance(widget, QCheckBox) and widget.objectName():
            ...
            radioName, radioValue = valueName.split(constants.CONFIG_NAME_MARKER)[
                1
            ].split(constants.CONFIG_VALUE_MARKER)
    ...
    def updateSubwidgets(self, container, parentwidget, subwidgets=None):
        ...
        for widget in container.children():
            ...
            if (
                hasattr(widget, "objectName")
                and widget.objectName()
                and widget.objectName() in subwidgets
            ):
    ...
    def addBasicTab(self):
        ...
        if self.datacleared == True:
            error = constants.ERROR_MESSAGE_MARKER + "{}".format(
                getMessage("gui-data-cleared-notification")
            )
            self.error = error
        ...
        self.connectionSettingsGroup = QtWidgets.QGroupBox(
            getMessage("connection-group-title")
        )
        ...
        self.editRoomsButton.setIcon(
            QtGui.QIcon(resourcespath + "bullet_edit_centered.png")
        )
        self.editRoomsButton.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "edit-rooms"
        )
        ...
        self.serverpassTextbox.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "password"
        )
        ...
        self.connectionSettingsLayout.addWidget(
            self.editRoomsButton, 3, 2, Qt.AlignRight
        )
        ...
        if isMacOS():
            self.connectionSettingsGroup.setFixedHeight(
                self.connectionSettingsGroup.minimumSizeHint().height()
            )
        else:
            self.connectionSettingsGroup.setMaximumHeight(
                self.connectionSettingsGroup.minimumSizeHint().height()
            )
        ...
        self.mediaplayerSettingsGroup = QtWidgets.QGroupBox(
            getMessage("media-setting-title")
        )
        ...
        self.executablepathCombobox.currentIndexChanged.connect(
            self.updateExecutableIcon
        )
        self.executablepathCombobox.setEditText(
            self._tryToFillPlayerPath(config["playerPath"], playerpaths)
        )
        ...
        self.executablebrowseButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "folder_explore.png"),
            getMessage("browse-label"),
        )
        ...
        self.mediabrowseButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "folder_explore.png"),
            getMessage("browse-label"),
        )
        ...
        self.executablepathCombobox.setMinimumContentsLength(
            constants.EXECUTABLE_COMBOBOX_MINIMUM_LENGTH
        )
        if not IsPySide6:
            self.executablepathCombobox.setSizeAdjustPolicy(
        ...
        self.mediapathTextbox.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "media-path"
        )
        self.playerargsLabel.setObjectName("player-arguments")
        self.playerargsTextbox.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "player-arguments"
        )
        ...
        self.mediaplayerSettingsLayout.addWidget(
            self.executablepathCombobox, 0, 2, 1, 1
        )
        self.mediaplayerSettingsLayout.addWidget(
            self.executablebrowseButton, 0, 3, 1, 1
        )
        ...
        iconWidth = (
            self.executableiconLabel.minimumSize().width()
            + self.mediaplayerSettingsLayout.spacing()
        )
        maxWidth = max(
        ...
        self.showmoreCheckbox.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "more"
        )
        ...
        self.basicOptionsFrame.setSizePolicy(
    ...
    def addReadinessTab(self):
        ...
        self.unpauseIfAlreadyReadyOption = QRadioButton(
            getMessage("unpause-ifalreadyready-option")
        )
        self.readyUnpauseButtonGroup.addButton(self.unpauseIfAlreadyReadyOption)
        self.unpauseIfAlreadyReadyOption.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        self.unpauseIfAlreadyReadyOption.setObjectName(
            "unpause-ifalreadyready"
            + constants.CONFIG_NAME_MARKER
            + "unpauseAction"
            + constants.CONFIG_VALUE_MARKER
            + constants.UNPAUSE_IFALREADYREADY_MODE
        )
        self.readyUnpauseLayout.addWidget(self.unpauseIfAlreadyReadyOption)
        self.unpauseIfOthersReadyOption = QRadioButton(
            getMessage("unpause-ifothersready-option")
        )
        self.readyUnpauseButtonGroup.addButton(self.unpauseIfOthersReadyOption)
        self.unpauseIfOthersReadyOption.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        self.unpauseIfOthersReadyOption.setObjectName(
            "unpause-ifothersready"
            + constants.CONFIG_NAME_MARKER
            + "unpauseAction"
            + constants.CONFIG_VALUE_MARKER
            + constants.UNPAUSE_IFOTHERSREADY_MODE
        )
        self.readyUnpauseLayout.addWidget(self.unpauseIfOthersReadyOption)
        self.unpauseIfMinUsersReadyOption = QRadioButton(
            getMessage("unpause-ifminusersready-option")
        )
        self.readyUnpauseButtonGroup.addButton(self.unpauseIfMinUsersReadyOption)
        self.unpauseIfMinUsersReadyOption.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        self.unpauseIfMinUsersReadyOption.setObjectName(
            "unpause-ifminusersready"
            + constants.CONFIG_NAME_MARKER
            + "unpauseAction"
            + constants.CONFIG_VALUE_MARKER
            + constants.UNPAUSE_IFMINUSERSREADY_MODE
        )
        ...
        self.unpauseAlwaysUnpauseOption.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        self.unpauseAlwaysUnpauseOption.setObjectName(
            "unpause-always"
            + constants.CONFIG_NAME_MARKER
            + "unpauseAction"
            + constants.CONFIG_VALUE_MARKER
            + constants.UNPAUSE_ALWAYS_MODE
        )
    ...
    def addMiscTab(self):
        ...
        self.filenameprivacySendRawOption = QRadioButton(
            getMessage("privacy-sendraw-option")
        )
        self.filenameprivacySendHashedOption = QRadioButton(
            getMessage("privacy-sendhashed-option")
        )
        self.filenameprivacyDontSendOption = QRadioButton(
            getMessage("privacy-dontsend-option")
        )
        ...
        self.filesizeprivacySendRawOption = QRadioButton(
            getMessage("privacy-sendraw-option")
        )
        self.filesizeprivacySendHashedOption = QRadioButton(
            getMessage("privacy-sendhashed-option")
        )
        self.filesizeprivacyDontSendOption = QRadioButton(
            getMessage("privacy-dontsend-option")
        )
        ...
        self.filenameprivacySendRawOption.setObjectName(
            "privacy-sendraw"
            + constants.CONFIG_NAME_MARKER
            + "filenamePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
            + constants.PRIVACY_SENDRAW_MODE
        )
        self.filenameprivacySendHashedOption.setObjectName(
            "privacy-sendhashed"
            + constants.CONFIG_NAME_MARKER
            + "filenamePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
            + constants.PRIVACY_SENDHASHED_MODE
        )
        self.filenameprivacyDontSendOption.setObjectName(
            "privacy-dontsend"
            + constants.CONFIG_NAME_MARKER
            + "filenamePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
            + constants.PRIVACY_DONTSEND_MODE
        )
        self.filesizeprivacyLabel.setObjectName("filesize-privacy")
        self.filesizeprivacySendRawOption.setObjectName(
            "privacy-sendraw"
            + constants.CONFIG_NAME_MARKER
            + "filesizePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
        ...
        self.filesizeprivacySendHashedOption.setObjectName(
            "privacy-sendhashed"
            + constants.CONFIG_NAME_MARKER
            + "filesizePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
        ...
        self.filesizeprivacyDontSendOption.setObjectName(
            "privacy-dontsend"
            + constants.CONFIG_NAME_MARKER
            + "filesizePrivacyMode"
            + constants.CONFIG_VALUE_MARKER
        ...
        self.coreSettingsLayout.addWidget(
            self.filenameprivacySendRawOption, 3, 1, Qt.AlignLeft
        )
        self.coreSettingsLayout.addWidget(
            self.filenameprivacySendHashedOption, 3, 2, Qt.AlignLeft
        )
        self.coreSettingsLayout.addWidget(
            self.filenameprivacyDontSendOption, 3, 3, Qt.AlignLeft
        )
        self.coreSettingsLayout.addWidget(self.filesizeprivacyLabel, 4, 0)
        self.coreSettingsLayout.addWidget(
            self.filesizeprivacySendRawOption, 4, 1, Qt.AlignLeft
        )
        self.coreSettingsLayout.addWidget(
            self.filesizeprivacySendHashedOption, 4, 2, Qt.AlignLeft
        )
        self.coreSettingsLayout.addWidget(
            self.filesizeprivacyDontSendOption, 4, 3, Qt.AlignLeft
        )

        ## Syncplay internals

        self.internalSettingsGroup = QtWidgets.QGroupBox(
            getMessage("syncplay-internals-title")
        )
        ...
        self.alwaysshowCheckbox.setObjectName(
            constants.INVERTED_STATE_MARKER + "forceGuiPrompt"
        )
        self.internalSettingsLayout.addWidget(self.alwaysshowCheckbox)

        self.automaticupdatesCheckbox = QCheckBox(
            getMessage("checkforupdatesautomatically-label")
        )
        ...
        self.autosaveJoinsToListCheckbox = QCheckBox(
            getMessage("autosavejoinstolist-label")
        )
        ...
        self.mediasearchSettingsGroup = QtWidgets.QGroupBox(
        ...
        self.mediasearchTextEdit = QPlainTextEdit(
            utils.getListAsMultilineString(self.mediaSearchDirectories)
        )
        self.mediasearchTextEdit.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "mediasearcdirectories-arguments"
        )
        ...
        self.mediasearchSettingsGroup.setMaximumHeight(
            self.mediasearchSettingsGroup.minimumSizeHint().height()
        )
    ...
    def addSyncTab(self):
        ...
        self.desyncSettingsGroup = QtWidgets.QGroupBox(
            getMessage("sync-otherslagging-title")
        )
        ...
        self.desyncSettingsLayout.addWidget(
            self.slowdownCheckbox, 0, 0, 1, 2, Qt.AlignLeft
        )
        self.desyncSettingsLayout.addWidget(
            self.rewindCheckbox, 1, 0, 1, 2, Qt.AlignLeft
        )
        ...
        self.othersyncSettingsGroup = QtWidgets.QGroupBox(
            getMessage("sync-youlaggging-title")
        )
        ...
        self.othersyncSettingsLayout.addWidget(
            self.dontslowwithmeCheckbox, 2, 0, 1, 2, Qt.AlignLeft
        )

        self.othersyncSettingsLayout.setAlignment(Qt.AlignLeft)
        self.othersyncSettingsLayout.addWidget(
            self.fastforwardCheckbox, 3, 0, 1, 2, Qt.AlignLeft
        )

        ## Trusted domains

        self.trusteddomainsSettingsGroup = QtWidgets.QGroupBox(
            getMessage("syncplay-trusteddomains-title")
        )
        ...
        self.trusteddomainsTextEdit = QPlainTextEdit(
            utils.getListAsMultilineString(self.trustedDomains)
        )
        self.trusteddomainsTextEdit.setObjectName(
            constants.LOAD_SAVE_MANUALLY_MARKER + "trusteddomains-arguments"
        )
        ...
        self.trusteddomainsSettingsGroup.setMaximumHeight(
            self.trusteddomainsSettingsGroup.minimumSizeHint().height()
        )

        self.othersyncSettingsGroup.setLayout(self.othersyncSettingsLayout)
        self.othersyncSettingsGroup.setMaximumHeight(
            self.othersyncSettingsGroup.minimumSizeHint().height()
        )
        ...
        self.desyncSettingsGroup.setMaximumHeight(
            self.desyncSettingsGroup.minimumSizeHint().height()
        )
    ...
    def addChatTab(self):
        ...
        self.chatInputLayout.addWidget(
            self.chatInputEnabledCheckbox, 1, 0, 1, 1, Qt.AlignLeft
        )
        ...
        self.chatDirectInputCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                self.posixresourcespath + "chevrons_right.png"
            )
        )
        self.chatInputLayout.addWidget(
            self.chatDirectInputCheckbox, 2, 0, 1, 1, Qt.AlignLeft
        )
        ...
        self.inputFontFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        self.chatFontLabel = QLabel(getMessage("chatinputfont-label"), self)
        self.chatFontLabel.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                self.posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.chatInputColourButton = QtWidgets.QPushButton(
            getMessage("chatcolour-label")
        )
        ...
        self.chatInputColourButton.released.connect(
            lambda: self.colourDialog("chatInput")
        )
        ...
        self.chatInputPositionFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        self.chatInputPositionLabel = QLabel(
            getMessage("chatinputposition-label"), self
        )
        self.chatInputPositionLabel.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                self.posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.chatInputTopOption.setObjectName(
            "chatinputposition-top"
            + constants.CONFIG_NAME_MARKER
            + "chatInputPosition"
            + constants.CONFIG_VALUE_MARKER
            + constants.INPUT_POSITION_TOP
        )
        self.chatInputMiddleOption.setObjectName(
            "chatinputposition-middle"
            + constants.CONFIG_NAME_MARKER
            + "chatInputPosition"
            + constants.CONFIG_VALUE_MARKER
            + constants.INPUT_POSITION_MIDDLE
        )
        self.chatInputBottomOption.setObjectName(
            "chatinputposition-bottom"
            + constants.CONFIG_NAME_MARKER
            + "chatInputPosition"
            + constants.CONFIG_VALUE_MARKER
            + constants.INPUT_POSITION_BOTTOM
        )
        ...
        self.subitems["chatInputEnabled"] = [
            self.chatInputPositionLabel.objectName(),
            self.chatInputTopOption.objectName(),
            self.chatInputMiddleOption.objectName(),
            self.chatInputBottomOption.objectName(),
            self.chatInputFontButton.objectName(),
            self.chatFontLabel.objectName(),
            self.chatInputColourButton.objectName(),
            self.chatDirectInputCheckbox.objectName(),
        ]
        # Output
        ...
        self.chatOutputEnabledCheckbox = QCheckBox(
            getMessage("chatoutputenabled-label")
        )
        self.chatOutputEnabledCheckbox.setObjectName("chatOutputEnabled")
        self.chatOutputLayout.addWidget(
            self.chatOutputEnabledCheckbox, 1, 0, 1, 1, Qt.AlignLeft
        )
        ...
        self.outputFontFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        self.chatOutputFontLabel = QLabel(getMessage("chatoutputfont-label"), self)
        self.chatOutputFontLabel.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                self.posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.chatOutputFontButton.released.connect(
            lambda: self.fontDialog("chatOutput")
        )
        self.chatOutputColourButton = QtWidgets.QPushButton(
            getMessage("chatcolour-label")
        )
        ...
        self.chatOutputModeLabel.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                self.posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.chatOutputScrollingOption = QRadioButton(
            getMessage("chat-scrolling-option")
        )
        ...
        self.chatOutputChatroomOption.setObjectName(
            "chatoutputmode-chatroom"
            + constants.CONFIG_NAME_MARKER
            + "chatOutputMode"
            + constants.CONFIG_VALUE_MARKER
            + constants.CHATROOM_MODE
        )
        self.chatOutputScrollingOption.setObjectName(
            "chatoutputmode-scrolling"
            + constants.CONFIG_NAME_MARKER
            + "chatOutputMode"
            + constants.CONFIG_VALUE_MARKER
            + constants.SCROLLING_MODE
        )
        ...
        self.chatOutputModeFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        ...
        self.subitems["chatOutputEnabled"] = [
            self.chatOutputModeLabel.objectName(),
            self.chatOutputChatroomOption.objectName(),
            self.chatOutputScrollingOption.objectName(),
            self.chatOutputFontButton.objectName(),
    ...
    def addMessageTab(self):
        ...
        self.showSameRoomOSDCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.showNonControllerOSDCheckbox = QCheckBox(
            getMessage("shownoncontrollerosd-label")
        )
        self.showNonControllerOSDCheckbox.setObjectName("showNonControllerOSD")
        self.showNonControllerOSDCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.showDifferentRoomOSDCheckbox = QCheckBox(
            getMessage("showdifferentroomosd-label")
        )
        self.showDifferentRoomOSDCheckbox.setObjectName("showDifferentRoomOSD")
        self.showDifferentRoomOSDCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.slowdownOSDCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.showOSDWarningsCheckbox.setStyleSheet(
            constants.STYLE_SUBCHECKBOX.format(
                posixresourcespath + "chevrons_right.png"
            )
        )
        ...
        self.osdSettingsGroup.setMaximumHeight(
            self.osdSettingsGroup.minimumSizeHint().height()
        )
        ...
        self.displaySettingsGroup = QtWidgets.QGroupBox(
            getMessage("messages-other-title")
        )
        self.displaySettingsLayout = QtWidgets.QVBoxLayout()
        self.displaySettingsFrame = QtWidgets.QFrame()

        self.showDurationNotificationCheckbox = QCheckBox(
            getMessage("showdurationnotification-label")
        )
        ...
        self.displaySettingsGroup.setMaximumHeight(
            self.displaySettingsGroup.minimumSizeHint().height()
        )
    ...
    def addBottomLayout(self):
        ...
        self.helpButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "help.png"), getMessage("help-label")
        )
        ...
        self.resetButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "cog_delete.png"), getMessage("reset-label")
        )
        ...
        self.runButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "accept.png"), getMessage("run-label")
        )
        ...
        self.storeAndRunButton = QtWidgets.QPushButton(
            QtGui.QIcon(resourcespath + "accept.png"), getMessage("storeandrun-label")
        )
        ...
        self.enableplaylistsCheckbox = QCheckBox(
            getMessage("sharedplaylistenabled-label")
        )
        self.bottomCheckboxLayout.addWidget(self.showmoreCheckbox)
        self.bottomCheckboxLayout.addWidget(
            self.enableplaylistsCheckbox, 0, 2, Qt.AlignRight
        )
        self.alwaysshowCheckbox.setObjectName(
            constants.INVERTED_STATE_MARKER + "forceGuiPrompt"
        )
    ...
    def tabList(self):
        ...
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "house.png"), getMessage("basics-label")
            )
        )
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "control_pause_blue.png"),
                getMessage("readiness-label"),
            )
        )
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "film_link.png"), getMessage("sync-label")
            )
        )
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "user_comment.png"),
                getMessage("chat-label"),
            )
        )
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "error.png"), getMessage("messages-label")
            )
        )
        self.tabListWidget.addItem(
            QtWidgets.QListWidgetItem(
                QtGui.QIcon(resourcespath + "cog.png"), getMessage("misc-label")
            )
        )
        self.tabListLayout.addWidget(self.tabListWidget)
        self.tabListFrame.setLayout(self.tabListLayout)
        self.tabListFrame.setFixedWidth(
            self.tabListFrame.minimumSizeHint().width() + constants.TAB_PADDING
        )
    ...
    def ensureTabListIsVisible(self):
        ...
        while (
            self.tabListWidget.horizontalScrollBar().isVisible()
            and self.tabListFrame.width() < constants.MAXIMUM_TAB_WIDTH
        ):
    ...
    def updatePasswordVisibilty(self):
        if (
            self.hostCombobox.currentText() == ""
            and self.serverpassTextbox.text() == ""
        ) or str(self.hostCombobox.currentText()) in self.publicServerAddresses:
    ...
    def __init__(self, config, playerpaths, error, defaultConfig):
        ...
        if isWindows():
            ...
            resourcespath = utils.findWorkingDir() + "/resources/"
        self.posixresourcespath = (
            utils.findWorkingDir().replace("\\", "/") + "/resources/"
        )
