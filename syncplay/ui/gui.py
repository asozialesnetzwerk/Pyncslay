...
try:
    if hasattr(QtCore.Qt, "AA_EnableHighDpiScaling"):
        QtWidgets.QApplication.setAttribute(
            QtCore.Qt.AA_EnableHighDpiScaling, applyDPIScaling
        )
    ...
    def paint(self, itemQPainter, optionQStyleOptionViewItem, indexQModelIndex):
        ...
        midY = int(
            (
                ...
                + optionQStyleOptionViewItem.rect.bottomLeft().y()
            )
            / 2
        )
        if column == constants.USERLIST_GUI_USERNAME_COLUMN:
            ...
            itemQModelIndex = currentQAbstractItemModel.index(
                indexQModelIndex.row(),
                constants.USERLIST_GUI_USERNAME_COLUMN,
                indexQModelIndex.parent(),
            )
            ...
            if isUserRow and (isMacOS() or isLinux()):
                blankRect = QtCore.QRect(
                    0,
                    optionQStyleOptionViewItem.rect.y(),
                    optionQStyleOptionViewItem.rect.width(),
                    optionQStyleOptionViewItem.rect.height(),
                )
            ...
            if isUserRow:
                optionQStyleOptionViewItem.rect.setX(
                    ...
                    + constants.USERLIST_GUI_USERNAME_OFFSET
                )
        if column == constants.USERLIST_GUI_FILENAME_COLUMN:
            currentQAbstractItemModel = indexQModelIndex.model()
            itemQModelIndex = currentQAbstractItemModel.index(
                indexQModelIndex.row(),
                constants.USERLIST_GUI_FILENAME_COLUMN,
                indexQModelIndex.parent(),
            )
            ...
            if fileSwitchRole == constants.FILEITEM_SWITCH_FILE_SWITCH:
                ...
                optionQStyleOptionViewItem.rect.setX(
                    optionQStyleOptionViewItem.rect.x() + 16
                )
        ...
        QtWidgets.QStyledItemDelegate.paint(
            self, itemQPainter, optionQStyleOptionViewItem, indexQModelIndex
        )
...
class AboutDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        if isDarkMode:
            linkLabel.setText(
                (
                    '<center><a href="https://syncplay.pl" style="{}">syncplay.pl</a></center>'
                ).format(constants.STYLE_DARK_ABOUT_LINK_COLOR)
            )
        else:
            linkLabel.setText(
                '<center><a href="https://syncplay.pl">syncplay.pl</a></center>'
            )
        linkLabel.setOpenExternalLinks(True)
        versionExtString = version + revision
        versionLabel = QtWidgets.QLabel(
            "<p><center>"
            + getMessage("about-dialog-release").format(
                versionExtString, release_number
            )
            + "<br />Python "
            + python_version()
            + " - "
        ...
        dependenciesButton = QtWidgets.QPushButton(
            getMessage("about-dialog-dependencies")
        )
    ...
    def openLicense(self):
        if isWindows():
            QtGui.QDesktopServices.openUrl(
                QUrl("file:///" + resourcespath + "license.rtf")
            )
        else:
            QtGui.QDesktopServices.openUrl(
                QUrl("file://" + resourcespath + "license.rtf")
    ...
    def openDependencies(self):
        if isWindows():
            QtGui.QDesktopServices.openUrl(
                QUrl("file:///" + resourcespath + "third-party-notices.txt")
            )
        else:
            QtGui.QDesktopServices.openUrl(
                QUrl("file://" + resourcespath + "third-party-notices.txt")
...
class CertificateDialog(QtWidgets.QDialog):
    def __init__(self, tlsData, parent=None):
        ...
        statusLabel = QtWidgets.QLabel(
            getMessage("tls-dialog-status-label").format(tlsData["subject"])
        )
        descLabel = QtWidgets.QLabel(
            getMessage("tls-dialog-desc-label").format(tlsData["subject"])
        )
        connDataLabel = QtWidgets.QLabel(
            getMessage("tls-dialog-connection-label").format(
                tlsData["protocolVersion"], tlsData["cipher"]
            )
        )
        certDataLabel = QtWidgets.QLabel(
            getMessage("tls-dialog-certificate-label").format(
                tlsData["issuer"], tlsData["expires"]
            )
        )
        if isMacOS():
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    class PlaylistItemDelegate(QtWidgets.QStyledItemDelegate):
        def paint(self, itemQPainter, optionQStyleOptionViewItem, indexQModelIndex):
            itemQPainter.save()
            currentQAbstractItemModel = indexQModelIndex.model()
            currentlyPlayingFile = currentQAbstractItemModel.data(
                indexQModelIndex,
                Qt.UserRole + constants.PLAYLISTITEM_CURRENTLYPLAYING_ROLE,
            )
            if currentlyPlayingFile:
                currentlyplayingIconQPixmap = QtGui.QPixmap(
                    resourcespath + "bullet_right_grey.png"
                )
                midY = int(
                    (
                        optionQStyleOptionViewItem.rect.y()
            ...
            if MainWindow.insertPosition == 0 and indexQModelIndex.row() == 0:
                ...
                MainWindow.insertPosition
                and indexQModelIndex.row() == MainWindow.insertPosition - 1
            ):
                lineBelow = True
            if lineAbove:
                line = QLine(
                    optionQStyleOptionViewItem.rect.topLeft(),
                    optionQStyleOptionViewItem.rect.topRight(),
                )
                itemQPainter.drawLine(line)
            elif lineBelow:
                line = QLine(
                    optionQStyleOptionViewItem.rect.bottomLeft(),
                    optionQStyleOptionViewItem.rect.bottomRight(),
                )
                itemQPainter.drawLine(line)
    ...
    class PlaylistGroupBox(QtWidgets.QGroupBox):
        ...
        def dropEvent(self, event):
            ...
            if QtGui.QDropEvent.proposedAction(event) == Qt.MoveAction:
                QtGui.QDropEvent.setDropAction(
                    event, Qt.CopyAction
                )  # Avoids file being deleted
            data = event.mimeData()
            urls = data.urls()
            ...
            if urls and urls[0].scheme() == "file":
                ...
                for url in urls[::-1]:
                    if isMacOS() and IsPySide:
                        ...
                        pathString = macURL.stringByAddingPercentEscapesUsingEncoding_(
                            NSUTF8StringEncoding
                        )
                        dropfilepath = os.path.abspath(
                            NSURL.URLWithString_(pathString).filePathURL().path()
                        )
                    else:
    ...
    class PlaylistWidget(QtWidgets.QListWidget):
        ...
        def updatePlaylistIndexIcon(self):
            for item in range(self.count()):
                ...
                if fileIsUntrusted:
                    if isDarkMode:
                        self.item(item).setForeground(
                            QtGui.QBrush(
                                QtGui.QColor(constants.STYLE_DARK_UNTRUSTEDITEM_COLOR)
                            )
                        )
                    else:
                        self.item(item).setForeground(
                            QtGui.QBrush(
                                QtGui.QColor(constants.STYLE_UNTRUSTEDITEM_COLOR)
                            )
                        )
                elif fileIsAvailable:
                    self.item(item).setForeground(
                        QtGui.QBrush(
                            self.selfWindow.palette().color(QtGui.QPalette.Text)
                        )
                    )
                else:
                    if isDarkMode:
                        self.item(item).setForeground(
                            QtGui.QBrush(
                                QtGui.QColor(constants.STYLE_DARK_DIFFERENTITEM_COLOR)
                            )
                        )
                    else:
                        self.item(item).setForeground(
                            QtGui.QBrush(
                                QtGui.QColor(constants.STYLE_DIFFERENTITEM_COLOR)
                            )
                        )
            self.selfWindow._syncplayClient.fileSwitch.setFilenameWatchlist(
                self.selfWindow.newWatchlist
            )
            self.forceUpdate()
    ...
    def addClient(self, client):
        ...
        try:
            ...
            self._syncplayClient.fileSwitch.setMediaDirectories(
                self.config["mediaSearchDirectories"]
            )
            if not self.config["mediaSearchDirectories"]:
                self._syncplayClient.ui.showErrorMessage(
                    getMessage("no-media-directories-error")
                )
    ...
    def showMessage(self, message, noTimestamp=False, isMotd=False):
        message = str(message)
        username = None
        messageWithUsername = re.match(
            constants.MESSAGE_WITH_USERNAME_REGEX, message, re.UNICODE
        )
        ...
        if username:
            message = constants.STYLE_USER_MESSAGE.format(
                constants.STYLE_USERNAME, username, message
            )
        ...
        if noTimestamp:
            ...
            self.newMessage(
                time.strftime(constants.UI_TIME_FORMAT, time.localtime())
                + message
                + "<br />"
            )
    ...
    def getFileSwitchState(self, filename):
        if filename:
            ...
            if self._syncplayClient.userlist.currentUser.file and utils.sameFilename(
    ...
    def showUserList(self, currentUser, rooms):
        ...
        self._usertreebuffer.setHorizontalHeaderLabels(
            (
                getMessage("roomuser-heading-label"),
                getMessage("size-heading-label"),
                getMessage("duration-heading-label"),
                getMessage("filename-heading-label"),
            )
        )
        usertreeRoot = self._usertreebuffer.invisibleRootItem()
        if (
            self._syncplayClient.userlist.currentUser.file
            and self._syncplayClient.userlist.currentUser.file
            and os.path.isfile(self._syncplayClient.userlist.currentUser.file["path"])
        ):
            self._syncplayClient.fileSwitch.setCurrentDirectory(
                os.path.dirname(self._syncplayClient.userlist.currentUser.file["path"])
            )
        ...
        for room in rooms:
            ...
            for user in rooms[room]:
                ...
                useritem.setData(
                    isController, Qt.UserRole + constants.USERITEM_CONTROLLER_ROLE
                )
                useritem.setData(
                    isReadyWithFile, Qt.UserRole + constants.USERITEM_READY_ROLE
                )
                if user.file:
                    filesizeitem = QtGui.QStandardItem(formatSize(user.file["size"]))
                    filedurationitem = QtGui.QStandardItem(
                        "({})".format(formatTime(user.file["duration"]))
                    )
                    ...
                    fileSwitchState = (
                        self.getFileSwitchState(user.file["name"])
                        if room == currentUser.room
                        else None
                    )
                    if fileSwitchState != constants.FILEITEM_SWITCH_NO_SWITCH:
                        filenameTooltip = getMessage("switch-to-file-tooltip").format(
                            filename
                        )
                    else:
                    ...
                    filenameitem.setData(
                        fileSwitchState, Qt.UserRole + constants.FILEITEM_SWITCH_ROLE
                    )
                    if currentUser.file:
                        sameName = sameFilename(
                            user.file["name"], currentUser.file["name"]
                        )
                        sameSize = sameFilesize(
                            user.file["size"], currentUser.file["size"]
                        )
                        sameDuration = sameFileduration(
                            user.file["duration"], currentUser.file["duration"]
                        )
                        underlinefont = QtGui.QFont()
                        underlinefont.setUnderline(True)
                        differentItemColor = (
                            constants.STYLE_DARK_DIFFERENTITEM_COLOR
                            if isDarkMode
                            else constants.STYLE_DIFFERENTITEM_COLOR
                        )
                        if sameRoom:
                            if not sameName:
                                filenameitem.setForeground(
                                    QtGui.QBrush(QtGui.QColor(differentItemColor))
                                )
                                filenameitem.setFont(underlinefont)
                            if not sameSize:
                                if formatSize(user.file["size"]) == formatSize(
                                    currentUser.file["size"]
                                ):
                                    filesizeitem = QtGui.QStandardItem(
                                        formatSize(user.file["size"], precise=True)
                                    )
                                filesizeitem.setFont(underlinefont)
                                filesizeitem.setForeground(
                                    QtGui.QBrush(QtGui.QColor(differentItemColor))
                                )
                            if not sameDuration:
                                filedurationitem.setForeground(
                                    QtGui.QBrush(QtGui.QColor(differentItemColor))
                                )
                    ...
                    if room == currentUser.room:
                        if isDarkMode:
                            filenameitem.setForeground(
                                QtGui.QBrush(
                                    QtGui.QColor(constants.STYLE_DARK_NOFILEITEM_COLOR)
                                )
                            )
                        else:
                            filenameitem.setForeground(
                                QtGui.QBrush(
                                    QtGui.QColor(constants.STYLE_NOFILEITEM_COLOR)
                                )
                            )
                font = QtGui.QFont()
                ...
                if isControlledRoom and not isController:
                    useritem.setForeground(
                        QtGui.QBrush(QtGui.QColor(constants.STYLE_NOTCONTROLLER_COLOR))
                    )
                ...
                roomitem.appendRow(
                    (useritem, filesizeitem, filedurationitem, filenameitem)
                )
    ...
    def openPlaylistMenu(self, position):
        ...
        if item:
            ...
            pathFound = (
                self._syncplayClient.fileSwitch.findFilepath(firstFile)
                if not isURL(firstFile)
                else None
            )
            if (
                self._syncplayClient.userlist.currentUser.file is None
                or firstFile != self._syncplayClient.userlist.currentUser.file["name"]
            ):
                if isURL(firstFile):
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "world_go.png"),
                        getMessage("openstreamurl-menu-label"),
                        lambda: self.openFile(
                            firstFile, resetPosition=True, fromUser=True
                        ),
                    )
                elif pathFound:
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "film_go.png"),
                        getMessage("openmedia-menu-label"),
                        lambda: self.openFile(
                            pathFound, resetPosition=True, fromUser=True
                        ),
                    )
            if pathFound:
                menu.addAction(
                    QtGui.QPixmap(resourcespath + "folder_film.png"),
            ...
            if self._syncplayClient.isUntrustedTrustableURI(firstFile):
                ...
                if domain:
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "shield_add.png"),
                        getMessage("addtrusteddomain-menu-label").format(domain),
                        lambda: self.addTrustedDomain(domain),
                    )
            menu.addAction(
                QtGui.QPixmap(resourcespath + "delete.png"),
                getMessage("removefromplaylist-menu-label"),
                lambda: self.deleteSelectedPlaylistItems(),
            )
            menu.addSeparator()
        menu.addAction(
            QtGui.QPixmap(resourcespath + "arrow_switch.png"),
            getMessage("shuffleremainingplaylist-menu-label"),
            lambda: self.shuffleRemainingPlaylist(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "arrow_switch.png"),
            getMessage("shuffleentireplaylist-menu-label"),
            lambda: self.shuffleEntirePlaylist(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "arrow_undo.png"),
            getMessage("undoplaylist-menu-label"),
            lambda: self.undoPlaylistChange(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "film_edit.png"),
            getMessage("editplaylist-menu-label"),
            lambda: self.openEditPlaylistDialog(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "film_add.png"),
            getMessage("addfilestoplaylist-menu-label"),
            lambda: self.OpenAddFilesToPlaylistDialog(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "world_add.png"),
            getMessage("addurlstoplaylist-menu-label"),
            lambda: self.OpenAddURIsToPlaylistDialog(),
        )
        menu.addSeparator()
        menu.addAction(
            getMessage("loadplaylistfromfile-menu-label"),
            lambda: self.OpenLoadPlaylistFromFileDialog(),
        )  # TODO: Add icon
        menu.addAction(
            "Load and shuffle playlist from file",
            lambda: self.OpenLoadPlaylistFromFileDialog(shuffle=True),
        )  # TODO: Add icon and messages_en
        menu.addAction(
            getMessage("saveplaylisttofile-menu-label"),
            lambda: self.OpenSavePlaylistToFileDialog(),
        )  # TODO: Add icon
        menu.addSeparator()
        menu.addAction(
            QtGui.QPixmap(resourcespath + "film_folder_edit.png"),
            getMessage("setmediadirectories-menu-label"),
            lambda: self.openSetMediaDirectoriesDialog(),
        )
        menu.addAction(
            QtGui.QPixmap(resourcespath + "shield_edit.png"),
            getMessage("settrusteddomains-menu-label"),
            lambda: self.openSetTrustedDomainsDialog(),
        )
    ...
    def openRoomMenu(self, position):
        ...
        if username == self._syncplayClient.userlist.currentUser.username:
            addUsersFileToPlaylistLabelText = getMessage(
                "addyourfiletoplaylist-menu-label"
            )
            addUsersStreamToPlaylistLabelText = getMessage(
                "addyourstreamstoplaylist-menu-label"
            )
        else:
            addUsersFileToPlaylistLabelText = getMessage(
                "addotherusersfiletoplaylist-menu-label"
            ).format(shortUsername)
            addUsersStreamToPlaylistLabelText = getMessage(
                "addotherusersstreamstoplaylist-menu-label"
            ).format(shortUsername)
        ...
        if roomToJoin != self._syncplayClient.getRoom():
            menu.addAction(
                getMessage("joinroom-menu-label").format(roomToJoin),
                lambda: self.joinRoom(roomToJoin),
            )
        elif username and filename and filename != getMessage("nofile-note"):
            if self.config["sharedPlaylistEnabled"] and not self.isItemInPlaylist(
                filename
            ):
                if isURL(filename):
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "world_add.png"),
                        addUsersStreamToPlaylistLabelText,
                        lambda: self.addStreamToPlaylist(filename),
                    )
                else:
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "film_add.png"),
                        addUsersFileToPlaylistLabelText,
                        lambda: self.addStreamToPlaylist(filename),
                    )

            if (
                self._syncplayClient.userlist.currentUser.file is None
                or filename != self._syncplayClient.userlist.currentUser.file["name"]
            ):
                if isURL(filename):
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "world_go.png"),
                        getMessage("openusersstream-menu-label").format(shortUsername),
                        lambda: self.openFile(
                            filename, resetPosition=False, fromUser=True
                        ),
                    )
                else:
                    ...
                    if pathFound:
                        menu.addAction(
                            QtGui.QPixmap(resourcespath + "film_go.png"),
                            getMessage("openusersfile-menu-label").format(
                                shortUsername
                            ),
                            lambda: self.openFile(
                                pathFound, resetPosition=False, fromUser=True
                            ),
                        )
            ...
            if not isURL(filename) and filename != getMessage("nofile-note"):
                ...
                if path:
                    menu.addAction(
                        QtGui.QPixmap(resourcespath + "folder_film.png"),
                        getMessage("open-containing-folder"),
                        lambda: utils.open_system_file_browser(path),
                    )

        if (
            roomToJoin == self._syncplayClient.getRoom()
            and self._syncplayClient.userlist.currentUser.canControl()
            and self._syncplayClient.userlist.isReadinessSupported(
                requiresOtherUsers=False
            )
            and self._syncplayClient.serverFeatures["setOthersReadiness"]
        ):
            if self._syncplayClient.userlist.isReady(username):
                addSetUserAsReadyText = getMessage("setasnotready-menu-label").format(
                    shortUsername
                )
                menu.addAction(
                    QtGui.QPixmap(resourcespath + "cross.png"),
                    addSetUserAsReadyText,
                    lambda: self._syncplayClient.setOthersReadiness(username, False),
                )
            else:
                addSetUserAsNotReadyText = getMessage("setasready-menu-label").format(
                    shortUsername
                )
                menu.addAction(
                    QtGui.QPixmap(resourcespath + "tick.png"),
                    addSetUserAsNotReadyText,
                    lambda: self._syncplayClient.setOthersReadiness(username, True),
                )
    ...
    def updateListGeometry(self):
        try:
            ...
            while self.listTreeModel.item(roomtocheck):
                self.listTreeView.setFirstColumnSpanned(
                    roomtocheck, self.listTreeView.rootIndex(), True
                )
                roomtocheck += 1
            self.listTreeView.header().setStretchLastSection(False)
            if IsPySide6 or IsPySide2:
                self.listTreeView.header().setSectionResizeMode(
            ...
            if IsPySide:
                self.listTreeView.header().setResizeMode(
                    0, QtWidgets.QHeaderView.ResizeToContents
                )
                self.listTreeView.header().setResizeMode(
                    1, QtWidgets.QHeaderView.ResizeToContents
            ...
            NarrowTabsWidth = (
                self.listTreeView.header().sectionSize(0)
                + self.listTreeView.header().sectionSize(1)
                + self.listTreeView.header().sectionSize(2)
            )
            if self.listTreeView.header().width() < (
                NarrowTabsWidth + self.listTreeView.header().sectionSize(3)
            ):
                self.listTreeView.header().resizeSection(
                    3, self.listTreeView.header().width() - NarrowTabsWidth
                )
            else:
                if IsPySide6 or IsPySide2:
                    self.listTreeView.header().setSectionResizeMode(
    ...
    def playlistItemClicked(self, item):
        ...
        if isURL(filename):
            ...
            pathFound = self._syncplayClient.fileSwitch.findFilepath(
                filename, highPriority=True
            )
            if pathFound:
                self._syncplayClient.openFile(pathFound, resetPosition=True)
            else:
                self._syncplayClient.ui.showErrorMessage(
                    getMessage("cannot-find-file-for-playlist-switch-error").format(
                        filename
                    )
                )
    ...
    def _isTryingToChangeToCurrentFile(self, filename):
        if (
            self._syncplayClient.userlist.currentUser.file
            and filename == self._syncplayClient.userlist.currentUser.file["name"]
        ):
            self.showDebugMessage(
                "File change request ignored (Syncplay should not be asked to change to current filename)"
            )
            return True
        else:
            return False
            ...
            if isURL(filename):
                ...
                if pathFound:
                    ...
                    self.showErrorMessage(
                        getMessage("switch-file-not-found-error").format(filename)
                    )
    ...
    def showErrorMessage(self, message, criticalerror=False):
        ...
        message = message.replace(
            "&lt;a href=&quot;https://syncplay.pl/trouble&quot;&gt;",
            '<a href="https://syncplay.pl/trouble">',
        ).replace("&lt;/a&gt;", "</a>")
        message = message.replace(
            "&lt;a href=&quot;https://mpv.io/&quot;&gt;", '<a href="https://mpv.io/">'
        ).replace("&lt;/a&gt;", "</a>")
        message = message.replace(
            "&lt;a href=&quot;https://github.com/stax76/mpv.net/&quot;&gt;",
            '<a href="https://github.com/stax76/mpv.net/">',
        ).replace("&lt;/a&gt;", "</a>")
        message = message.replace("\n", "<br />")
        if isDarkMode:
            message = (
                '<span style="{}">'.format(constants.STYLE_DARK_ERRORNOTIFICATION)
                + message
                + "</span>"
            )
        else:
            message = (
                '<span style="{}">'.format(constants.STYLE_ERRORNOTIFICATION)
                + message
                + "</span>"
            )
        self.newMessage(
    ...
    def seekPositionDialog(self):
        seekTime, ok = QtWidgets.QInputDialog.getText(
            ...
            getMessage("seektime-msgbox-label"),
    ...
    def getInitialMediaDirectory(self, includeUserSpecifiedDirectories=True):
        if IsPySide:
            if (
                self.config["mediaSearchDirectories"]
                and os.path.isdir(self.config["mediaSearchDirectories"][0])
                and includeUserSpecifiedDirectories
            ):
                ...
                defaultdirectory = QtGui.QDesktopServices.storageLocation(
                    QtGui.QDesktopServices.MoviesLocation
                )
            elif os.path.isdir(
                QtGui.QDesktopServices.storageLocation(
            ...
            if (
                ...
                defaultdirectory = QStandardPaths.standardLocations(
    ...
    def browseMediapath(self):
        ...
        fileName, filtr = QtWidgets.QFileDialog.getOpenFileName(
            self,
            getMessage("browseformedia-label"),
            defaultdirectory,
            browserfilter,
            "",
            options,
    ...
    def OpenLoadPlaylistFromFileDialog(self, shuffle=False):
        ...
        filepath, filtr = QtWidgets.QFileDialog.getOpenFileName(
            ...
            "",
            options,
        )  # TODO: Note Shuffle and move to messages_en
        if os.path.isfile(filepath):
            self._syncplayClient.playlist.loadPlaylistFromFile(
    ...
    def OpenSavePlaylistToFileDialog(self):
        ...
        filepath, filtr = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save playlist to file", defaultdirectory, browserfilter, "", options
    ...
    def OpenAddURIsToPlaylistDialog(self):
        ...
        URIsButtonBox.setStandardButtons(
        ...
        URIsDialog.setWindowFlags(
            URIsDialog.windowFlags() & ~Qt.WindowContextHelpButtonHint
        )
        URIsDialog.show()
    ...
    def openEditRoomsDialog(self):
        ...
        RoomsTextbox.setPlainText(
            utils.getListAsMultilineString(self.config["roomList"])
        )
        ...
        RoomsButtonBox.setStandardButtons(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        ...
        RoomsDialog.setWindowFlags(
            RoomsDialog.windowFlags() & ~Qt.WindowContextHelpButtonHint
        )
        RoomsDialog.show()
    ...
    def openEditPlaylistDialog(self):
        ...
        editPlaylistButtonBox.setStandardButtons(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        ...
        editPlaylistDialog.setWindowFlags(
            editPlaylistDialog.windowFlags() & ~Qt.WindowContextHelpButtonHint
        )
        editPlaylistDialog.show()
        result = editPlaylistDialog.exec_()
        if result == QtWidgets.QDialog.Accepted:
            newPlaylist = utils.convertMultilineStringToList(
                editPlaylistTextbox.toPlainText()
            )
            if (
                newPlaylist != self.playlistState
                and self._syncplayClient
                and not self.updatingPlaylist
            ):
    ...
    def openSetMediaDirectoriesDialog(self):
        ...
        MediaDirectoriesDialog.setWindowTitle(
            getMessage("syncplay-mediasearchdirectories-title")
        )  # TODO: Move to messages_*.py
        MediaDirectoriesLayout = QtWidgets.QGridLayout()
        MediaDirectoriesLabel = QtWidgets.QLabel(
            getMessage("syncplay-mediasearchdirectories-label")
        )
        ...
        MediaDirectoriesTextbox.setPlainText(
            utils.getListAsMultilineString(self.config["mediaSearchDirectories"])
        )
        ...
        MediaDirectoriesButtonBox.setStandardButtons(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        ...
        MediaDirectoriesAddFolderButton = QtWidgets.QPushButton(
            getMessage("addfolder-label")
        )
        MediaDirectoriesAddFolderButton.pressed.connect(
            lambda: self.openAddMediaDirectoryDialog(
                MediaDirectoriesTextbox, MediaDirectoriesDialog
            )
        )
        MediaDirectoriesLayout.addWidget(
            MediaDirectoriesAddFolderButton, 1, 1, 1, 1, Qt.AlignTop
        )
        MediaDirectoriesDialog.setLayout(MediaDirectoriesLayout)
        MediaDirectoriesDialog.setWindowFlags(
            MediaDirectoriesDialog.windowFlags() & ~Qt.WindowContextHelpButtonHint
        )
        MediaDirectoriesDialog.setModal(True)
        MediaDirectoriesDialog.show()
        result = MediaDirectoriesDialog.exec_()
        if result == QtWidgets.QDialog.Accepted:
            newMediaDirectories = utils.convertMultilineStringToList(
                MediaDirectoriesTextbox.toPlainText()
            )
    ...
    def openSetTrustedDomainsDialog(self):
        ...
        TrustedDomainsLabel = QtWidgets.QLabel(
            getMessage("trusteddomains-msgbox-label")
        )
        ...
        TrustedDomainsTextbox.setPlainText(
            utils.getListAsMultilineString(self.config["trustedDomains"])
        )
        ...
        TrustedDomainsButtonBox.setStandardButtons(
            QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel
        )
        ...
        TrustedDomainsDialog.setWindowFlags(
            TrustedDomainsDialog.windowFlags() & ~Qt.WindowContextHelpButtonHint
        )
        TrustedDomainsDialog.setModal(True)
        TrustedDomainsDialog.show()
        result = TrustedDomainsDialog.exec_()
        if result == QtWidgets.QDialog.Accepted:
            newTrustedDomains = utils.convertMultilineStringToList(
                TrustedDomainsTextbox.toPlainText()
            )
    ...
    @needsClient
    def openAddMediaDirectoryDialog(
        self, MediaDirectoriesTextbox, MediaDirectoriesDialog
    ):
        if isMacOS() and IsPySide:
            options = QtWidgets.QFileDialog.Options(
        ...
        folderName = str(
            QtWidgets.QFileDialog.getExistingDirectory(
                self,
                None,
                self.getInitialMediaDirectory(includeUserSpecifiedDirectories=False),
                options,
            )
        )
    ...
    def promptForStreamURL(self):
        streamURL, ok = QtWidgets.QInputDialog.getText(
            self,
            getMessage("promptforstreamurl-msgbox-label"),
            getMessage("promptforstreamurlinfo-msgbox-label"),
            QtWidgets.QLineEdit.Normal,
            "",
        )
        if ok and streamURL != "":
    ...
    def createControlledRoom(self):
        controlroom, ok = QtWidgets.QInputDialog.getText(
            self,
            getMessage("createcontrolledroom-msgbox-label"),
            getMessage("controlledroominfo-msgbox-label"),
            QtWidgets.QLineEdit.Normal,
    ...
    def identifyAsController(self):
        ...
        controlpassword, ok = QtWidgets.QInputDialog.getText(
            self, msgboxtitle, msgboxtext, QtWidgets.QLineEdit.Normal, ""
        )
    ...
    def setOffset(self):
        ...
        newoffset, ok = QtWidgets.QInputDialog.getText(
            self,
            getMessage("setoffset-msgbox-label"),
            getMessage("offsetinfo-msgbox-label"),
            QtWidgets.QLineEdit.Normal,
            oldoffset,
        )
        if ok and newoffset != "":
    ...
    def openUserGuide(self):
        if isLinux():
            self.QtGui.QDesktopServices.openUrl(
                QUrl("https://syncplay.pl/guide/linux/")
            )
        elif isWindows():
            self.QtGui.QDesktopServices.openUrl(
                QUrl("https://syncplay.pl/guide/windows/")
            )
        else:
    ...
    def addTopLayout(self, window):
        ...
        if isDarkMode:
            window.outputbox.document().setDefaultStyleSheet(
                constants.STYLE_DARK_LINKS_COLOR
            )
        window.outputbox.setReadOnly(True)
        window.outputbox.setTextInteractionFlags(
            window.outputbox.textInteractionFlags() | Qt.TextSelectableByKeyboard
        )
        window.outputbox.setOpenExternalLinks(True)
        window.outputbox.unsetCursor()
        window.outputbox.moveCursor(QtGui.QTextCursor.End)
        window.outputbox.insertHtml(
            constants.STYLE_CONTACT_INFO.format(getMessage("contact-label"))
        )
        window.outputbox.moveCursor(QtGui.QTextCursor.End)
        ...
        window.chatFrame.setSizePolicy(
        ...
        if isMacOS():
            window.outputLayout.setSpacing(8)
        ...
        if isMacOS():
            ...
            window.sslButton = QtWidgets.QPushButton(
                QtGui.QPixmap(resourcespath + "lock_green.png").scaled(14, 14), ""
            )
            ...
            window.sslButton.setStyleSheet(
                "QPushButton:!hover{border: 1px solid gray;} QPushButton:hover{border:2px solid black;}"
            )
        else:
            window.listlabel.setMinimumHeight(27)
            window.sslButton = QtWidgets.QPushButton(
                QtGui.QPixmap(resourcespath + "lock_green.png"), ""
            )
            window.sslButton.setVisible(False)
        ...
        window.listFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
        window.listLayout.setContentsMargins(0, 0, 0, 0)
        if isMacOS():
            window.listLayout.setSpacing(8)
        ...
        window.userlistFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
        ...
        if isMacOS():
            window.userlistLayout.setContentsMargins(3, 0, 3, 0)
        ...
        window.roomButton = QtWidgets.QPushButton(
            QtGui.QPixmap(resourcespath + "door_in.png"), getMessage("joinroom-label")
        )
        ...
        window.roomFrame.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        if isMacOS():
        ...
        if isMacOS():
            window.listFrame.setMinimumHeight(window.outputFrame.height())
    ...
    def addBottomLayout(self, window):
        ...
        if isMacOS():
            window.bottomLayout.setSpacing(0)

        self.addPlaybackLayout(window)

        window.playlistGroup = self.PlaylistGroupBox(
            getMessage("sharedplaylistenabled-label")
        )
        ...
        window.playlistGroup.setSizePolicy(
            QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Preferred
        )
        ...
        playlistItem = QtWidgets.QListWidgetItem(
            getMessage("playlist-instruction-item-message")
        )
        ...
        if isMacOS():
            window.listLayout.setContentsMargins(0, 0, 0, 10)
        ...
        if isMacOS():
            ...
            window.autoplayPushButton.setSizePolicy(
        ...
        window.autoplayLabel.setSizePolicy(
            QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Minimum
        )
        window.autoplayLabel.setMaximumWidth(
            window.autoplayLabel.minimumSizeHint().width()
        )
        window.autoplayLabel.setToolTip(getMessage("autoplay-tooltip"))
        window.autoplayThresholdSpinbox = QtWidgets.QSpinBox()
        window.autoplayThresholdSpinbox.setMaximumWidth(
            window.autoplayThresholdSpinbox.minimumSizeHint().width()
        )
        ...
        window.autoplayThresholdSpinbox.valueChanged.connect(
            self.changeAutoplayThreshold
        )
        ...
        window.bottomFrame.setMaximumHeight(
            window.bottomFrame.minimumSizeHint().height()
        )
    ...
    def addPlaybackLayout(self, window):
        ...
        window.seekButton = QtWidgets.QPushButton(
        ...
        window.unseekButton = QtWidgets.QPushButton(
            QtGui.QPixmap(resourcespath + "arrow_undo.png"), ""
        )
        ...
        window.playButton = QtWidgets.QPushButton(
        ...
        window.pauseButton = QtWidgets.QPushButton(
    ...
    def populateMenubar(self, window):
        ...
        window.openAction = window.fileMenu.addAction(
            QtGui.QPixmap(resourcespath + "folder_explore.png"),
            getMessage("openmedia-menu-label"),
        )
        window.openAction.triggered.connect(self.browseMediapath)
        window.openAction = window.fileMenu.addAction(
            QtGui.QPixmap(resourcespath + "world_explore.png"),
            getMessage("openstreamurl-menu-label"),
        )
        ...
        window.setTrustedDomainsAction.triggered.connect(
        ...
        window.identifyascontroller = window.advancedMenu.addAction(
            QtGui.QPixmap(resourcespath + "key_go.png"),
        ...
        window.editroomsAction = window.windowMenu.addAction(
            QtGui.QPixmap(resourcespath + "door_open_edit.png"),
            getMessage("roomlist-msgbox-label"),
        )
        window.editroomsAction.triggered.connect(self.openEditRoomsDialog)
        window.menuBar.addMenu(window.windowMenu)

        window.playbackAction = window.windowMenu.addAction(
            getMessage("playbackbuttons-menu-label")
        )
        window.playbackAction.setCheckable(True)
        window.playbackAction.triggered.connect(self.updatePlaybackFrameVisibility)

        window.autoplayAction = window.windowMenu.addAction(
            getMessage("autoplay-menu-label")
        )
        window.autoplayAction.setCheckable(True)
        window.autoplayAction.triggered.connect(self.updateAutoplayVisibility)

        window.hideEmptyRoomsAction = window.windowMenu.addAction(
            getMessage("hideemptyrooms-menu-label")
        )
    ...
    def changeAutoplayThreshold(self, source=None):
        self._syncplayClient.changeAutoPlayThrehsold(
            self.autoplayThresholdSpinbox.value()
        )
    ...
    def changeAutoplayState(self, source=None):
        self.updateAutoPlayIcon()
        if self._syncplayClient:
            self._syncplayClient.changeAutoplayState(
                self.autoplayPushButton.isChecked()
            )
        else:
    ...
    def updateReadyIcon(self):
        ...
        if ready:
            self.readyPushButton.setIcon(
                QtGui.QPixmap(resourcespath + "tick_checkbox.png")
            )
        else:
            self.readyPushButton.setIcon(
                QtGui.QPixmap(resourcespath + "empty_checkbox.png")
            )

    def updateAutoPlayIcon(self):
        ready = self.autoplayPushButton.isChecked()
        if ready:
            self.autoplayPushButton.setIcon(
                QtGui.QPixmap(resourcespath + "tick_checkbox.png")
            )
        else:
            self.autoplayPushButton.setIcon(
    ...
    def automaticUpdateCheck(self):
        ...
        try:
            if self.config["lastCheckedForUpdates"]:
                configLastChecked = datetime.strptime(
                    self.config["lastCheckedForUpdates"], "%Y-%m-%d %H:%M:%S.%f"
                )
                if (
                    self.lastCheckedForUpdates is None
                    or configLastChecked > self.lastCheckedForUpdates.toPython()
                ):
                    self.lastCheckedForUpdates = QDateTime.fromString(
                        self.config["lastCheckedForUpdates"], "yyyy-MM-dd HH-mm-ss"
                    )
            if self.lastCheckedForUpdates is None:
                self.checkForUpdates()
            else:
                timeDelta = (
                    currentDateTimeValue.toPython()
                    - self.lastCheckedForUpdates.toPython()
                )
                if (
            ...
            self.showDebugMessage(
                "Automatic check for updates failed. An update check was manually trigggered. Reason: {}".format(
                    str(e)
                )
            )
            self.checkForUpdates()
    ...
    def checkForUpdates(self, userInitiated=False):
        ...
        updateStatus, updateMessage, updateURL, self.publicServerList = (
            self._syncplayClient.checkForUpdate(userInitiated)
        )
        ...
        if updateMessage is None:
            if updateStatus == "uptodate":
                ...
                updateMessage = getMessage("update-check-failed-notification").format(
                    syncplay.version
                )
    ...
    def dropEvent(self, event):
        ...
        if urls and urls[0].scheme() == "file":
            ...
            if isMacOS() and IsPySide:
                ...
                dropfilepath = os.path.abspath(str(url.toLocalFile()))
            if rewindFile == False:
                self._syncplayClient.openFile(
                    dropfilepath, resetPosition=False, fromUser=True
                )
            else:
                self._syncplayClient.setPosition(0)
                self._syncplayClient.openFile(
                    dropfilepath, resetPosition=True, fromUser=True
                )
                self._syncplayClient.setPosition(0)
    ...
    def setPlaylist(self, newPlaylist, newIndexFilename=None):
        if self.updatingPlaylist:
            self.ui.showDebugMessage(
                "Trying to set playlist while it is already being updated"
            )
    ...
    def addFileToPlaylist(self, filePath, index=-1):
        if not isURL(filePath):
            ...
            if self.noPlaylistDuplicates(filename):
                ...
                self._syncplayClient.fileSwitch.notifyUserIfFileNotInMediaDirectory(
                    filename, filePath
                )
        else:
            self.removePlaylistNote()
    ...
    def noPlaylistDuplicates(self, filename):
        if self.isItemInPlaylist(filename):
            self.showErrorMessage(
                getMessage("cannot-add-duplicate-error").format(filename)
            )
            return False
        else:
            return True
    ...
    def addFolderToPlaylist(self, folderPath):
        self.showErrorMessage(
            "You tried to add the folder '{}' to the playlist. Syncplay only currently supports adding files to the playlist.".format(
                folderPath
            )
        )  # TODO: Implement "add folder to playlist"
    ...
    def loadSettings(self):
        ...
        posIsOnScreen = windowGeometry.contains(
            QtCore.QRect(movePos.x(), movePos.y(), 1, 1)
        )
        ...
        self.autoplayThresholdSpinbox.blockSignals(True)
        self.autoplayThresholdSpinbox.setValue(
            int(settings.value("autoplayMinUsers", 2))
        )
        self.autoplayThresholdSpinbox.blockSignals(False)
        settings.endGroup()
        settings = QSettings("Syncplay", "Interface")
