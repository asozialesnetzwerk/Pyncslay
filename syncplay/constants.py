...
def getValueForOS(constantDict):
    if sys.platform.startswith(OS_WINDOWS):
        return (
            ...
            if OS_WINDOWS in constantDict
            else constantDict[OS_DEFAULT]
        )
    if sys.platform.startswith(OS_LINUX):
        return (
            constantDict[OS_LINUX]
            if OS_LINUX in constantDict
            else constantDict[OS_DEFAULT]
        )
    if sys.platform.startswith(OS_MACOS):
        return (
            constantDict[OS_MACOS]
            if OS_MACOS in constantDict
            else constantDict[OS_DEFAULT]
        )
    if OS_BSD in sys.platform or sys.platform.startswith(OS_DRAGONFLY):
        return (
            constantDict[OS_BSD] if OS_BSD in constantDict else constantDict[OS_DEFAULT]
        )
...
RECENT_CLIENT_THRESHOLD = (
    "1.7.4"  # This and higher considered 'recent' clients (no warnings)
)
MUSIC_FORMATS = [
    ".mp3",
    ".m4a",
...
PLAYLIST_LOAD_NEXT_FILE_TIME_FROM_END_THRESHOLD = (
    5  # Seconds (only triggered if file is paused, e.g. due to EOF)
)
EXECUTABLE_COMBOBOX_MINIMUM_LENGTH = (
    30  # Minimum number of characters that the combobox will make visible
)
...
SHOW_SLOWDOWN_OSD = (
    True  # Show notifications of slowing down / reverting on time difference
)
SHOW_SAME_ROOM_OSD = (
    True  # Show OSD notifications for events relating to room user is in
)
SHOW_NONCONTROLLER_OSD = (
    False  # Show OSD notifications for non-controllers in controlled rooms
)
SHOW_DIFFERENT_ROOM_OSD = (
    False  # Show OSD notifications for events relating to room user is not in
)
...
MONOSPACE_FONT = getValueForOS(
    {OS_DEFAULT: "Monospace", OS_MACOS: "Menlo", OS_WINDOWS: "Consolas"}
)
...
FOLDER_SEARCH_WARNING_THRESHOLD = (
    2.0  # Secs - how long until a warning saying how many files have been scanned
)
...
try:
    ...
    MPVNET_PATHS.append(
        os.path.expandvars(r"%LOCALAPPDATA%\Microsoft\WindowsApps\mpvnet.exe")
    )
    MPVNET_PATHS.append(
...
USERLIST_GUI_USERNAME_OFFSET = getValueForOS({OS_DEFAULT: 21, OS_MACOS: 26})  # Pixels
...
MPV_SYNCPLAYINTF_OPTIONS_TO_SEND = [
    "chatInputEnabled",
    "chatInputFontFamily",
...
MPV_SYNCPLAYINTF_LANGUAGE_TO_SEND = [
    "mpv-key-tab-hint",
    "mpv-key-hint",
