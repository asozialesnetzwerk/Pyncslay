...
class SyncplayClient(object):
    def __init__(self, playerClass, ui, config):
        ...
        constants.FOLDER_SEARCH_FIRST_FILE_TIMEOUT = config[
            "folderSearchFirstFileTimeout"
        ]
        constants.FOLDER_SEARCH_TIMEOUT = config["folderSearchTimeout"]
        constants.FOLDER_SEARCH_DOUBLE_CHECK_INTERVAL = config[
            "folderSearchDoubleCheckInterval"
        ]
        constants.FOLDER_SEARCH_WARNING_THRESHOLD = config[
        ...
        self._warnings = self._WarningManager(
            self._player, self.userlist, self.ui, self
        )
        ...
        if (
            constants.LIST_RELATIVE_CONFIGS
            and "loadedRelativePaths" in self._config
            and self._config["loadedRelativePaths"]
        ):
            paths = "; ".join(self._config["loadedRelativePaths"])
            self.ui.showMessage(
                getMessage("relative-config-notification").format(paths),
                noPlayer=True,
                noTimestamp=True,
            )
    ...
    def initPlayer(self, player):
        ...
        if not self._player.alertOSDSupported:
            constants.OSD_WARNING_MESSAGE_DURATION = (
                constants.NO_ALERT_OSD_WARNING_DURATION
            )
    ...
    def checkIfConnected(self):
        if (
            ...
            and self._protocol
            and time.time() - self._lastGlobalUpdate > constants.PROTOCOL_TIMEOUT
        ):
            self._lastGlobalUpdate = None
    ...
    def _determinePlayerStateChange(self, paused, position):
        pauseChange = (
            self.getPlayerPaused() != paused and self.getGlobalPaused() != paused
        )
        _playerDiff = abs(self.getPlayerPosition() - position)
        _globalDiff = abs(self.getGlobalPosition() - position)
        seeked = (
            _playerDiff > constants.SEEK_THRESHOLD
            and _globalDiff > constants.SEEK_THRESHOLD
        )
    ...
    def updatePlayerStatus(self, paused, position):
        ...
        currentLength = (
            self.userlist.currentUser.file["duration"]
            if self.userlist.currentUser.file
            else 0
        )
        if (
            pauseChange
            and paused
            and currentLength > constants.PLAYLIST_LOAD_NEXT_FILE_MINIMUM_LENGTH
            and abs(position - currentLength)
            < constants.PLAYLIST_LOAD_NEXT_FILE_TIME_FROM_END_THRESHOLD
        ):
            self.playlist.advancePlaylistCheck()
        elif (
            pauseChange
            and "readiness" in self.serverFeatures
            and self.serverFeatures["readiness"]
        ):
            if (
                currentLength == 0
                or currentLength == -1
                or not (
                    not self.playlist.notJustChangedPlaylist()
                    and abs(position - currentLength)
                    < constants.PLAYLIST_LOAD_NEXT_FILE_TIME_FROM_END_THRESHOLD
                )
            ):
            ...
            if (pauseChange or seeked) and self._protocol:
                if self.recentlyRewound() or self._recentlyAdvanced():
                    self._protocol.sendState(
                        self._globalPosition, self.getPlayerPaused(), False, None, True
                    )
                    return
                if seeked:
                    self.playerPositionBeforeLastSeek = self.getGlobalPosition()
                self._protocol.sendState(
                    self.getPlayerPosition(), self.getPlayerPaused(), seeked, None, True
                )

    def prepareToChangeToNewPlaylistItemAndRewind(self):
        self.ui.showDebugMessage(
            "Preparing to change to new playlist index and rewind..."
        )
        self.fileOpenBeforeChangingPlaylistIndex = (
            self.userlist.currentUser.file["path"]
            if self.userlist.currentUser.file
            else None
        )
    ...
    def prepareToAdvancePlaylist(self):
        if self.playlist.canSwitchToNextPlaylistIndex():
            ...
            self.ui.showDebugMessage(
                "Not preparing to advance playlist because the next file cannot be switched to"
            )

    def _recentlyAdvanced(self):
        lastAdvandedDiff = (
            time.time() - self.lastAdvanceTime if self.lastAdvanceTime else None
        )
        if (
            lastAdvandedDiff is not None
            and lastAdvandedDiff < constants.AUTOPLAY_DELAY + 5
        ):
            return True

    def recentlyConnected(self):
        connectDiff = (
            time.time() - self.lastConnectTime if self.lastConnectTime else None
        )
    ...
    def recentlyRewound(self, recentRewindThreshold=5.0):
        ...
        if (
            ...
            and self.lastUpdatedFileTime > lastRewindTime
        ):
            lastRewindTime = self.lastRewindTime - 4.5
        return (
            lastRewindTime is not None
            and abs(time.time() - lastRewindTime) < recentRewindThreshold
        )
    ...
    def _toggleReady(self, pauseChange, paused):
        if not self.userlist.currentUser.canControl():
            ...
            if not self.recentlyRewound() and not (
                (self._globalPaused == True) and not self._recentlyAdvanced()
            ):
            ...
            self.ui.showDebugMessage(
                "Readiness toggle ignored due to seamless music override"
            )
            self._player.setPaused(paused)
            self._playerPaused = paused
        elif (
            self.recentlyRewound()
            and (self._globalPaused == True)
            and not self._recentlyAdvanced()
        ):
            self._player.setPaused(self._globalPaused)
            self._playerPaused = self._globalPaused
            pauseChange = False
            ...
            lastPausedDiff = (
                time.time() - self.lastPausedOnLeaveTime
                if self.lastPausedOnLeaveTime
                else None
            )
            if (
                lastPausedDiff is not None
                and lastPausedDiff < constants.LAST_PAUSED_DIFF_THRESHOLD
            ):
                self.lastPausedOnLeaveTime = None
            else:
                self.changeReadyState(
                    not self.getPlayerPaused(), manuallyInitiated=False
                )
        return pauseChange
    ...
    def _rewindPlayerDueToTimeDifference(self, position, setBy):
        ...
        if self.getUsername() == setBy:
            self.ui.showDebugMessage(
                "Caught attempt to rewind due to time difference with self"
            )
        else:
            hideFromOSD = not constants.SHOW_SAME_ROOM_OSD
            self.setPosition(position)
            self.ui.showMessage(
                getMessage("rewind-notification").format(setBy), hideFromOSD
            )
            madeChangeOnPlayer = True
        return madeChangeOnPlayer
    ...
    def _fastforwardPlayerDueToTimeDifference(self, position, setBy):
        ...
        if self.getUsername() == setBy:
            self.ui.showDebugMessage(
                "Caught attempt to fastforward due to time difference with self"
            )
        else:
            hideFromOSD = not constants.SHOW_SAME_ROOM_OSD
            self.setPosition(position + constants.FASTFORWARD_EXTRA_TIME)
            self.ui.showMessage(
                getMessage("fastforward-notification").format(setBy), hideFromOSD
            )
            madeChangeOnPlayer = True
        return madeChangeOnPlayer
    ...
    def _serverUnpaused(self, setBy):
        ...
        madeChangeOnPlayer = True
        self.ui.showMessage(
            getMessage("unpause-notification").format(setBy), hideFromOSD
        )
        return madeChangeOnPlayer
    ...
    def _serverPaused(self, setBy):
        ...
        madeChangeOnPlayer = True
        if (
            self.lastLeftTime < time.time() - constants.OSD_DURATION
        ) or hideFromOSD == True:
            self.ui.showMessage(
                getMessage("pause-notification").format(
                    setBy, utils.formatTime(self.getGlobalPosition())
                ),
                hideFromOSD,
            )
        else:
            self.ui.showMessage(
                getMessage("left-paused-notification").format(self.lastLeftUser, setBy),
                hideFromOSD,
            )
        return madeChangeOnPlayer
    ...
    def _serverSeeked(self, position, setBy):
        ...
        if self.getUsername() != setBy:
            ...
            madeChangeOnPlayer = False
        message = getMessage("seek-notification").format(
            setBy,
            utils.formatTime(self.playerPositionBeforeLastSeek),
    ...
    def _slowDownToCoverTimeDifference(self, diff, setBy):
        ...
        if self._config["slowdownThreshold"] < diff and not self._speedChanged:
            if self.getUsername() == setBy:
                self.ui.showDebugMessage(
                    "Caught attempt to slow down due to time difference with self"
                )
            else:
                self._player.setSpeed(constants.SLOWDOWN_RATE)
                self._speedChanged = True
                self.ui.showMessage(
                    getMessage("slowdown-notification").format(setBy), hideFromOSD
                )
                madeChangeOnPlayer = True
    ...
    def _changePlayerStateAccordingToGlobalState(self, position, paused, doSeek, setBy):
        madeChangeOnPlayer = False
        pauseChanged = (
            paused != self.getGlobalPaused() or paused != self.getPlayerPaused()
        )
        ...
        if (
            diff > self._config["rewindThreshold"]
            and not doSeek
            and not self._config["rewindOnDesync"] == False
        ):
            madeChangeOnPlayer = self._rewindPlayerDueToTimeDifference(position, setBy)
        if self._config["fastforwardOnDesync"] and (
            self.userlist.currentUser.canControl() == False
            or self._config["dontSlowDownWithMe"] == True
        ):
            if diff < (constants.FASTFORWARD_BEHIND_THRESHOLD * -1) and not doSeek:
                if self.behindFirstDetected is None:
                    ...
                    if (
                        durationBehind
                        > (
                            self._config["fastforwardThreshold"]
                            - constants.FASTFORWARD_BEHIND_THRESHOLD
                        )
                    ) and (diff < (self._config["fastforwardThreshold"] * -1)):
                        madeChangeOnPlayer = self._fastforwardPlayerDueToTimeDifference(
                            position, setBy
                        )
                        self.behindFirstDetected = (
                            time.time() + constants.FASTFORWARD_RESET_THRESHOLD
                        )
            else:
                self.behindFirstDetected = None
        if (
            self._player.speedSupported
            and not doSeek
            and not paused
            and not self._config["slowOnDesync"] == False
        ):
    ...
    def updateGlobalState(self, position, paused, doSeek, setBy, messageAge):
        ...
        if not paused:
            position += messageAge
        if self._player:
            madeChangeOnPlayer = self._changePlayerStateAccordingToGlobalState(
                position, paused, doSeek, setBy
            )
    ...
    def setUserOffset(self, time):
        ...
        self.setPosition(self.getGlobalPosition())
        self.ui.showMessage(
            getMessage("current-offset-notification").format(self._userOffset)
        )
    ...
    def eofReportedByPlayer(self):
        if self.playlist.notJustChangedPlaylist() and self.userlist.currentUser.file:
            self.ui.showDebugMessage(
                "Fixing file duration to allow for playlist advancement"
            )
        ...
        if self._config["trustedDomains"]:
            for entry in self._config["trustedDomains"]:
                ...
                if o.hostname in (trustedDomain, "www." + trustedDomain):
                    ...
                    wildcardRegex = (
                        "^(" + re.escape(trustedDomain).replace("\\*", "([^.]+)") + ")$"
                    )
                    wildcardMatch = bool(
                        re.fullmatch(wildcardRegex, o.hostname, re.IGNORECASE)
                    )
                    if wildcardMatch:
                        foundMatch = True
                if not foundMatch:
    ...
    def openFile(self, filePath, resetPosition=False, fromUser=False):
        if not (filePath.startswith("http://") or filePath.startswith("https://")) and (
            (fromUser and filePath.endswith(".txt"))
            or filePath.endswith(".m3u")
            or filePath.endswith(".m3u8")
        ):
    ...
    def checkForFeatureSupport(self, featureList):
        self.serverFeatures = {
            "featureList": utils.meetsMinVersion(
                self.serverVersion, constants.FEATURE_LIST_MIN_VERSION
            ),
            "sharedPlaylists": utils.meetsMinVersion(
                self.serverVersion, constants.SHARED_PLAYLIST_MIN_VERSION
            ),
            "chat": utils.meetsMinVersion(
                self.serverVersion, constants.CHAT_MIN_VERSION
            ),
            "readiness": utils.meetsMinVersion(
                self.serverVersion, constants.USER_READY_MIN_VERSION
            ),
            "managedRooms": utils.meetsMinVersion(
                self.serverVersion, constants.CONTROLLED_ROOMS_MIN_VERSION
            ),
            ...
            "setOthersReadiness": utils.meetsMinVersion(
                self.serverVersion, constants.SET_OTHERS_READINESS_MIN_VERSION
            ),
        }
        if featureList:
            self.serverFeatures.update(featureList)
        if not utils.meetsMinVersion(
            self.serverVersion, constants.SHARED_PLAYLIST_MIN_VERSION
        ):
            self.ui.showErrorMessage(
                getMessage("shared-playlists-not-supported-by-server-error").format(
                    constants.SHARED_PLAYLIST_MIN_VERSION, self.serverVersion
                )
            )
        elif not self.serverFeatures["sharedPlaylists"]:
            self.ui.showErrorMessage(
                getMessage("shared-playlists-disabled-by-server-error")
            )
        # TODO: Have messages for all unsupported & disabled features
        if self.serverFeatures["maxChatMessageLength"] is not None:
            constants.MAX_CHAT_MESSAGE_LENGTH = self.serverFeatures[
        ...
        constants.MPV_SYNCPLAYINTF_CONSTANTS_TO_SEND = [
            ...
            "inputPromptStartCharacter={}".format(
                constants.MPV_INPUT_PROMPT_START_CHARACTER
            ),
            "inputPromptEndCharacter={}".format(
                constants.MPV_INPUT_PROMPT_END_CHARACTER
            ),
    ...
    def setUsername(self, username):
        if username and username != "":
            ...
            self.userlist.currentUser.username = "Anonymous" + str(
                random_number
            )  # Not localised as this would give away locale
    ...
    def getFeatures(self):
        ...
        features["sharedPlaylists"] = (
            self.sharedPlaylistIsEnabled()
        )  # Can change during runtime
    ...
    def isConnectedAndInARoom(self):
        return (
            self._protocol and self._protocol.logged and self.userlist.currentUser.room
        )

    def sharedPlaylistIsEnabled(self):
        if (
            "sharedPlaylists" in self.serverFeatures
            and not self.serverFeatures["sharedPlaylists"]
        ):
    ...
    def connected(self):
        ...
        readyState = (
            self._config["readyAtStart"]
            if self.userlist.currentUser.isReady() is None
            else self.userlist.currentUser.isReady()
        )
    ...
    def setPosition(self, position):
        if self._lastPlayerUpdate:
            self._lastPlayerUpdate = time.time()
        if (
            self.lastRewindTime is not None
            and abs(time.time() - self.lastRewindTime) < 1.0
            and position > 5
        ):
        ...
        if self._player and self.userlist.currentUser.file:
            if position < 0:
                position = 0
                self._protocol.sendState(
                    self.getPlayerPosition(), self.getPlayerPaused(), True, None, True
                )
    ...
    def start(self, host, port):
        ...
        if self._playerClass:
            perPlayerArguments = utils.getPlayerArgumentsByPathAsArray(
                self._config["perPlayerArguments"], self._config["playerPath"]
            )
            ...
            reactor.callLater(
                0.1,
                self._playerClass.run,
                self,
                self._config["playerPath"],
                filePath,
                self._config["playerArgs"],
            )
        ...
        try:
            ...
            trustRoot = trustRootFromCertificates(
                [Certificate.loadPEM(str(cert)) for cert in certs]
            )
            self.protocolFactory.options = optionsForClientTLS(
                hostname=host, trustRoot=trustRoot
            )
        ...
        def retry(retries):
            ...
            if retries > constants.RECONNECT_RETRIES:
                reactor.callLater(
                    0.1,
                    self.ui.showErrorMessage,
                    getMessage("connection-failed-notification"),
                    True,
                )
        ...
        self._reconnectingService = ClientService(
            self._endpoint, self.protocolFactory, retryPolicy=retry
        )
        try:
        ...
        def connectedNow(f):
            ...
            self.ui.showMessage(
                getMessage("reachout-successful-notification").format(host, hostIP)
            )
            return
        ...
        def requireServerFeatureDecorator(f):
            ...
            def wrapper(self, *args, **kwds):
                if self.serverVersion == "0.0.0":
                    self.ui.showDebugMessage(
                        "Tried to check server version too soon (testing support for: {})".format(
                            featureRequired
                        )
                    )
                    return None
                if (
                    featureRequired not in self.serverFeatures
                    or not self.serverFeatures[featureRequired]
                ):
                    featureName = getMessage("feature-{}".format(featureRequired))
                    self.ui.showErrorMessage(
                        getMessage("not-supported-by-server-error").format(featureName)
                    )
                    return
    ...
    def instaplayConditionsMet(self):
        ...
        if (
            ...
            or unpauseAction == constants.UNPAUSE_ALWAYS_MODE
        ):
            return True
        elif (
            unpauseAction == constants.UNPAUSE_IFOTHERSREADY_MODE
            and self.userlist.areAllOtherUsersInRoomReady()
        ):
            return True
        elif (
            unpauseAction == constants.UNPAUSE_IFMINUSERSREADY_MODE
            and self.userlist.areAllOtherUsersInRoomReady()
            and self.autoPlayThreshold
            and self.userlist.usersInRoomCount() >= self.autoPlayThreshold
        ):
            return True
        else:
            return False
    ...
    def autoplayConditionsMet(self):
        ...
        return (
            self._playerPaused
            and (self.autoPlay or recentlyAdvanced)
            and self.userlist.currentUser.canControl()
            and self.userlist.isReadinessSupported()
            and self.userlist.areAllUsersInRoomReady(
                requireSameFilenames=self._config["autoplayRequireSameFilenames"]
            )
            and (
                (
    ...
    def autoplayCountdown(self):
        if not self.autoplayConditionsMet():
            self.stopAutoplayCountdown()
            return
        allReadyMessage = getMessage("all-users-ready").format(
            self.userlist.readyUserCount()
        )
        autoplayingMessage = getMessage("autoplaying-notification").format(
            int(self.autoplayTimeLeft)
        )
        countdownMessage = "{}{}{}".format(
            allReadyMessage, self._player.osdMessageSeparator, autoplayingMessage
        )
        self.ui.showOSDMessage(
            countdownMessage,
            1,
            OSDType=constants.OSD_ALERT,
            mood=constants.MESSAGE_GOODNEWS,
        )
    ...
    def toggleReady(self, manuallyInitiated=True):
        self._protocol.setReady(
            not self.userlist.currentUser.isReady(), manuallyInitiated
        )
    ...
    def setReady(self, username, isReady, manuallyInitiated=True, setBy=None):
        ...
        if setBy:
            if isReady:
                self.ui.showMessage(
                    getMessage("other-set-as-ready-notification").format(
                        username, setBy
                    )
                )
            else:
                self.ui.showMessage(
                    getMessage("other-set-as-not-ready-notification").format(
                        username, setBy
                    )
                )
    ...
    def controlledRoomCreated(self, roomName, controlPassword):
        self.ui.showMessage(
            getMessage("created-controlled-room-notification").format(
                roomName, controlPassword, roomName, roomName + ":" + controlPassword
            )
        )
    ...
    def stripControlPassword(self, controlPassword):
        if controlPassword:
            return re.sub(
                constants.CONTROL_PASSWORD_STRIP_REGEX, "", controlPassword
            ).upper()
        else:
            return ""
    ...
    def identifyAsController(self, controlPassword):
        ...
        self.ui.showMessage(
            getMessage("identifying-as-controller-notification").format(controlPassword)
        )
        self.lastControlPasswordAttempt = controlPassword
    ...
    def controllerIdentificationError(self, username, room):
        if username == self.getUsername():
            self.ui.showErrorMessage(
                getMessage("failed-to-identify-as-controller-notification").format(
                    username
                )
            )
    ...
    def controllerIdentificationSuccess(self, username, roomname):
        ...
        if self.userlist.isRoomSame(roomname):
            hideFromOSD = not constants.SHOW_SAME_ROOM_OSD
            self.ui.showMessage(
                getMessage("authenticated-as-controller-notification").format(username),
                hideFromOSD,
            )
    ...
    def checkForUpdate(self, userInitiated):
        try:
            ...
            return (
                ...
                response["version-message"] if "version-message" in response else None,
                response["version-url"] if "version-url" in response else None,
                publicServers,
            )
        except Exception as e:
            return (
                ...
                str(e)
                + "\n-----\n"
                + getMessage("update-check-failed-notification").format(
                    syncplay.version
                ),
                constants.SYNCPLAY_DOWNLOAD_URL,
                None,
            )
    ...
    class _WarningManager(object):
        ...
        def _checkRoomForSameFiles(self, OSDOnly):
            if not self._userlist.areAllFilesInRoomSame():
                ...
                if (
                    not OSDOnly
                    and constants.SHOW_OSD_WARNINGS
                    and not self._warnings["room-file-differences"]["timer"].running
                ):
                    self._warnings["room-file-differences"]["timer"].start(
                        constants.WARNING_OSD_MESSAGES_LOOP_INTERVAL, True
                    )
        ...
        def _checkIfYouReAloneInTheRoom(self, OSDOnly):
            if self._userlist.areYouAloneInRoom():
                self._ui.showOSDMessage(
                    getMessage("alone-in-the-room"),
                    constants.WARNING_OSD_MESSAGES_LOOP_INTERVAL,
                    OSDType=constants.OSD_ALERT,
                ...
                if not OSDOnly:
                    ...
                    if (
                        constants.SHOW_OSD_WARNINGS
                        and not self._warnings["alone-in-the-room"]["timer"].running
                    ):
                        self._warnings["alone-in-the-room"]["timer"].start(
                            constants.WARNING_OSD_MESSAGES_LOOP_INTERVAL, True
                        )
        ...
        def checkReadyStates(self):
            if not self._client:
                return
            if (
                self._client.getPlayerPaused()
                or not self._userlist.currentUser.isReady()
                or not self._userlist.areAllRelevantUsersInRoomReady()
            ):
            ...
            if self._userlist.areYouAloneInRoom():
                ...
                if (
                    constants.SHOW_OSD_WARNINGS
                    and not self._warnings["not-all-ready"]["timer"].running
                ):
                    self._warnings["not-all-ready"]["timer"].start(
                        constants.WARNING_OSD_MESSAGES_LOOP_INTERVAL, True
                    )
        ...
        def _displayReadySameWarning(self):
            ...
            if (
                not self._userlist.areAllFilesInRoomSame()
                and fileDifferencesForRoom is not None
            ):
                messageMood = constants.MESSAGE_BADNEWS
                fileDifferencesMessage = getMessage("room-file-differences").format(
                    fileDifferencesForRoom
                )
                if (
                    self._userlist.currentUser.canControl()
                    and self._userlist.isReadinessSupported()
                ):
                    if self._userlist.areAllUsersInRoomReady():
                        allReadyMessage = getMessage("all-users-ready").format(
                            self._userlist.readyUserCount()
                        )
                        osdMessage = "{}{}{}".format(
                            fileDifferencesMessage,
                            self._client._player.osdMessageSeparator,
                            allReadyMessage,
                        )
                    else:
                        notAllReadyMessage = getMessage("not-all-ready").format(
                            self._userlist.usersInRoomNotReady()
                        )
                        osdMessage = "{}{}{}".format(
                            fileDifferencesMessage,
                            self._client._player.osdMessageSeparator,
                ...
                if self._userlist.areAllUsersInRoomReady():
                    osdMessage = getMessage("all-users-ready").format(
                        self._userlist.readyUserCount()
                    )
                else:
                    messageMood = constants.MESSAGE_BADNEWS
                    osdMessage = getMessage("not-all-ready").format(
                        self._userlist.usersInRoomNotReady()
                    )
            if osdMessage:
                self._ui.showOSDMessage(
        ...
        def __displayMessageOnOSD(self, warningName, warningFunction):
            if (
                constants.OSD_WARNING_MESSAGE_DURATION
                > self._warnings[warningName]["displayedFor"]
            ):
                warningFunction()
                self._warnings[warningName][
                    "displayedFor"
                ] += constants.WARNING_OSD_MESSAGES_LOOP_INTERVAL
            else:
        ...
        def __displayPausedMessagesOnOSD(self):
            ...
            if self._client and self._client._player and self._client.getPlayerPaused():
                ...
                not self._userlist.currentUser.isReady()
            ):  # CurrentUser should always be reminded they are set to not ready
                self.checkReadyStates()
...
class SyncplayUser(object):
    ...
    def setFile(self, filename, duration, size, path=None):
        file_ = {"name": filename, "duration": duration, "size": size, "path": path}
        self.file = file_
    ...
    def canControl(self):
        if self.isController() or not utils.RoomPasswordProvider.isControlledRoom(
            self.room
        ):
            return True
        else:
...
class SyncplayUserlist(object):
    ...
    def isReadinessSupported(self, requiresOtherUsers=True):
        if not utils.meetsMinVersion(
            self._client.serverVersion, constants.USER_READY_MIN_VERSION
        ):
            return False
    ...
    def __showUserChangeMessage(self, username, room, file_, oldRoom=None):
        if room:
            ...
            if (
                constants.SHOW_NONCONTROLLER_OSD == False
                and self.canControl(username) == False
            ):
                showOnOSD = False
            ...
            if not file_:
                ...
                message = getMessage("playing-notification").format(
                    username, file_["name"], duration
                )
                if (
                    self.currentUser.room != room
                    or self.currentUser.username == username
                ):
                    message += getMessage("playing-notification/room-addendum").format(
                        room
                    )
                self.ui.showMessage(message, hideFromOSD)
                if (
                    self.currentUser.file
                    and not self.currentUser.isFileSame(file_)
                    and self.currentUser.room == room
                ):
                    fileDifferences = self.getFileDifferencesForUser(
                        self.currentUser.file, file_
                    )
                    if fileDifferences is not None:
                        message = getMessage("file-differences-notification").format(
                            fileDifferences
                        )
    ...
    def getFileDifferencesForUser(self, currentUserFile, otherUserFile):
        if not currentUserFile or not otherUserFile:
            return None
        differences = []
        differentName = not utils.sameFilename(
            currentUserFile["name"], otherUserFile["name"]
        )
        differentSize = not utils.sameFilesize(
            currentUserFile["size"], otherUserFile["size"]
        )
        differentDuration = not utils.sameFileduration(
            currentUserFile["duration"], otherUserFile["duration"]
        )
        if differentName:
            differences.append(getMessage("file-difference-filename"))
        if differentSize:
            differences.append(getMessage("file-difference-filesize"))
        if differentDuration:
            differences.append(getMessage("file-difference-duration"))
    ...
    def getFileDifferencesForRoom(self):
        ...
        for otherUser in self._users.values():
            if otherUser.room == self.currentUser.room and otherUser.file:
                if not utils.sameFilename(
                    self.currentUser.file["name"], otherUser.file["name"]
                ):
                    differentName = True
                if not utils.sameFilesize(
                    self.currentUser.file["size"], otherUser.file["size"]
                ):
                    differentSize = True
                if not utils.sameFileduration(
                    self.currentUser.file["duration"], otherUser.file["duration"]
                ):
            ...
            for user in self._users.values():
                if user.room == self.currentUser.room and user.canControl():
                    if user.isReadyWithFile() == False:
                        ...
                        or not utils.sameFilename(
                            self.currentUser.file["name"], user.file["name"]
                        )
                    ):
                        return False
        return True
    ...
    def areAllFilesInRoomSame(self):
        if self.currentUser.file:
            for user in self._users.values():
                if (
                    user.room == self.currentUser.room
                    and user.file
                    and not self.currentUser.isFileSame(user.file)
                ):
                    if user.canControl():
                        return False
        return True
    ...
    def onlyUserInRoomWhoSupportsReadiness(self):
        for user in self._users.values():
            if (
                user.room == self.currentUser.room
                and user.isReadyWithFile() is not None
            ):
                return False
        return True
    ...
    def sortList(self, rooms):
        ...
        rooms = collections.OrderedDict(
            sorted(list(rooms.items()), key=lambda s: s[0].lower())
        )
        return rooms
...
class UiManager(object):
    ...
    def showChatMessage(self, username, userMessage):
        ...
        if (
            self._client._player.chatOSDSupported
            and self._client._config["chatOutputEnabled"]
        ):
    ...
    def showMessage(
        ...
        noPlayer=False,
        noTimestamp=False,
        OSDType=constants.OSD_NOTIFICATION,
        mood=constants.MESSAGE_NEUTRAL,
        isMotd=False,
    ):
        if not noPlayer:
            self.showOSDMessage(
                message, duration=constants.OSD_DURATION, OSDType=OSDType, mood=mood
            )
    ...
    def showOSDMessage(
        self,
        message,
        duration=constants.OSD_DURATION,
        OSDType=constants.OSD_NOTIFICATION,
        mood=constants.MESSAGE_NEUTRAL,
    ):
        ...
        if (
            OSDType == constants.OSD_ALERT
            and not constants.SHOW_OSD_WARNINGS
            and not self._client.autoplayTimerIsRunning()
        ):
            return
                ...
                if OSDType == constants.OSD_ALERT:
                    ...
                    if autoplayConditionsMet:
                        ...
                        self.lastAlertOSDEndTime = (
                            time.time() + constants.NO_ALERT_OSD_WARNING_DURATION
                        )
                    if (
                        self.lastNotificationOSDEndTime
                        and time.time() < self.lastNotificationOSDEndTime
                    ):
                        message = "{}{}{}".format(
                            message,
                            self._client._player.osdMessageSeparator,
                            self.lastNotificatinOSDMessage,
                        )
                else:
                    self.lastNotificatinOSDMessage = message
                    self.lastNotificationOSDEndTime = (
                        time.time() + constants.OSD_DURATION
                    )
                    if (
                        self.lastAlertOSDEndTime
                        ...
                        message = "{}{}{}".format(
                            self.lastAlertOSDMessage,
                            self._client._player.osdMessageSeparator,
                            message,
                        )
            self._client._player.displayMessage(
                message, int(duration * 1000), OSDType, mood
            )
...
class SyncplayPlaylist:
        ...
        def wrapper(self, *args, **kwds):
            if not self._client.sharedPlaylistIsEnabled():
                self._ui.showDebugMessage(
                    "Tried to use shared playlists when it was disabled!"
                )
                return
            return f(self, *args, **kwds)
    ...
    def loadDelayedPath(self, changeToIndex):
        ...
        if self._client.playerIsNotReady():
            self._client.addPlayerReadyCallback(
                lambda x: self.loadDelayedPath(changeToIndex)
            )
            return

        if (
            self._client._protocol.hadFirstPlaylistIndex
            and self._client.delayedLoadPath
        ):
            ...
            if self._client.sharedPlaylistIsEnabled():
                ...
                if len(self._playlist) == 0:
                    self._client.openFile(
                        delayedLoadPath, resetPosition=True, fromUser=True
                    )
                    ...
                    if currentPlaylistFilename != pathWithoutDirs:
                        if pathWithoutDirs not in self._playlist:
                            if utils.isURL(delayedLoadPath) or utils.isURL(
                                currentPlaylistFilename
                            ):
                                self._client.ui.addFileToPlaylist(delayedLoadPath)
                            else:
                                foundFilePath = self._client.fileSwitch.findFilepath(
                                    currentPlaylistFilename, highPriority=True
                                )
                                if foundFilePath is None:
                                    self._client.openFile(
                                        delayedLoadPath, resetPosition=False
                                    )
                                else:
                                    ...
                                    reactor.callLater(
                                        constants.DELAYED_LOAD_WAIT_TIME,
                                        self._client.ui.addFileToPlaylist,
                                        delayedLoadPath,
                                    )  # TODO: Avoid arbitary pause
                        else:
                            self._client.ui.showErrorMessage(
                                getMessage("cannot-add-duplicate-error").format(
                                    pathWithoutDirs
                                )
                            )
    ...
    def changeToPlaylistIndex(self, index, username=None, resetPosition=False):
        ...
        if self._client.playerIsNotReady():
            if not self.addedChangeListCallback:
                ...
                self._client.addPlayerReadyCallback(
                    lambda x: self.changeToPlaylistIndex(index, username, resetPosition)
                )
            return
        try:
            ...
            if (
                ...
                and self._client.userlist.currentUser.file
                and filename == self._client.userlist.currentUser.file["name"]
            ):
                self._playlistIndex = index
                return
        except IndexError:
        ...
        if username is None:
            if (
                self._client.isConnectedAndInARoom()
                and self._client.sharedPlaylistIsEnabled()
            ):
            ...
            self._ui.showMessage(
                getMessage("playlist-selection-changed-notification").format(username)
            )
        ...
        if self._thereIsNextPlaylistIndex() and self._client.sharedPlaylistIsEnabled():
            try:
                ...
                if utils.isURL(filename):
                    ...
                    path = self._client.fileSwitch.findFilepath(
    ...
    def switchToNewPlaylistIndex(self, index, resetPosition=False):
        try:
            ...
            self._ui.showDebugMessage(
                "Failed to find index {} in playlist".format(index)
            )
        ...
        if self._client.playerIsNotReady():
            self._client.addPlayerReadyCallback(
                lambda x: self.switchToNewPlaylistIndex(index, resetPosition)
            )
            return
        ...
        try:
            ...
            if utils.isURL(filename):
                if self._client.isURITrusted(filename):
                    ...
                    self._ui.showErrorMessage(
                        getMessage("cannot-add-unsafe-path-error").format(filename)
                    )
                return
            else:
            ...
            self._ui.showDebugMessage(
                "Could not change playlist index due to IndexError"
            )
    ...
    def _getValidIndexFromNewPlaylist(self, newPlaylist=None):
        ...
        while i > 0:
            try:
                ...
                validIndex = newPlaylist.index(filename)
                return (
                    validIndex + 1 if validIndex < len(newPlaylist) - 1 else validIndex
                )
            except:
                i -= 1
        return 0
    ...
    def loadPlaylistFromFile(self, path, shuffle=False):
        if not os.path.isfile(path):
            self._ui.showDebugMessage(
                "Not loading {} as file could not be found".format(path)
            )
            return
    ...
    def playlistNeedsRestoring(self, files, username):
        if self._client.playlistMayNeedRestoring:
            ...
            return (
                self._client.sharedPlaylistIsEnabled()
                and self._playlist != None
                and files == []
                and username == None
                and not self._playlistBufferIsFromOldRoom(
                    self._client.userlist.currentUser.room
                )
            )
    ...
    def changePlaylist(self, files, username=None, resetIndex=False):
        ...
        self._updateUndoPlaylistBuffer(
            newPlaylist=files, newRoom=self._client.userlist.currentUser.room
        )
        ...
        if username is None:
            if (
                ...
                self._ui.showMessage(
                    getMessage("playlist-contents-changed-notification").format(
                        self._client.getUsername()
                    )
                )
        else:
            self._ui.setPlaylist(self._playlist)
            self._ui.showMessage(
                getMessage("playlist-contents-changed-notification").format(username)
            )
    ...
    def canUndoPlaylist(self, currentPlaylist):
        return (
            self._previousPlaylist is not None
            and currentPlaylist != self._previousPlaylist
        )
    ...
    def advancePlaylistCheck(self):
        ...
        currentLength = (
            self._client.userlist.currentUser.file["duration"]
            if self._client.userlist.currentUser.file
            else 0
        )
        if (
            currentLength > constants.PLAYLIST_LOAD_NEXT_FILE_MINIMUM_LENGTH
            and abs(position - currentLength)
            < constants.PLAYLIST_LOAD_NEXT_FILE_TIME_FROM_END_THRESHOLD
    ...
    def _notPlayingCurrentIndex(self):
        if (
            self._playlistIndex is None
            or self._playlist is None
            or len(self._playlist) <= self._playlistIndex
        ):
            self._ui.showDebugMessage(
        ...
        if (
            ...
            and currentPlaylistFilename
            == self._client.userlist.currentUser.file["name"]
        ):
            return False
        else:
            self._ui.showDebugMessage(
                "Not playing current index - Filename mismatch or no file"
            )
            return True
    ...
    def _playlistBufferNeedsUpdating(self, newPlaylist):
        return (
            self._previousPlaylist != self._playlist and self._playlist != newPlaylist
        )
...
class FileSwitchManager(object):
    ...
    def changeMediaDirectories(self, mediaDirs):
        ...
        self._client.ui.showMessage(
            getMessage("media-directory-list-updated-notification")
        )
        self.mediaDirectoriesNotFound = []
        self.folderSearchEnabled = True
    ...
    def updateInfo(self):
        if not self.currentlyUpdating and self.mediaDirectories:
            threads.deferToThread(self._updateInfoThread).addCallback(
                lambda x: self.checkForFileSwitchUpdate()
            )
        ...
        with self.lock:
            try:
                ...
                if dirsToSearch:
                    ...
                    randomFilename = (
                        "RandomFile" + str(random.randrange(10000, 99999)) + ".txt"
                    )
                    for directory in dirsToSearch:
                        if not os.path.isdir(directory):
                            self.directorySearchError = getMessage(
                                "cannot-find-directory-error"
                            ).format(directory)
                        ...
                        if (
                            time.time() - startTime
                            > constants.FOLDER_SEARCH_FIRST_FILE_TIMEOUT
                        ):
                            self.folderSearchEnabled = False
                            self.directorySearchError = getMessage(
                                "folder-search-first-file-timeout-error"
                            ).format(directory)
                            return
                        ...
                        for root, dirs, files in os.walk(directory):
                            ...
                            if timeTakenSoFar > constants.FOLDER_SEARCH_TIMEOUT:
                                reactor.callLater(
                                    0.1,
                                    self._client.ui.showErrorMessage,
                                    getMessage("folder-search-timeout-error").format(
                                        directory, fileCount
                                    ),
                                    False,
                                )
                            ...
                            if (
                                timeTakenSoFar
                                > constants.FOLDER_SEARCH_WARNING_THRESHOLD
                            ):
                                if (
                                    not lastWarningTime
                                    or timeTakenSoFar - lastWarningTime >= 1
                                ):
                                    reactor.callLater(
                                        0.1,
                                        self._client.ui.showErrorMessage,
                                        getMessage(
                                            "folder-search-timeout-warning"
                                        ).format(
                                            int(timeTakenSoFar), fileCount, directory
                                        ),
                                        False,
                                    )
    ...
    def findFilepath(self, filename, highPriority=False):
        ...
        if self._client.userlist.currentUser.file and utils.sameFilename(
            filename, self._client.userlist.currentUser.file["name"]
        ):
    ...
    def notifyUserIfFileNotInMediaDirectory(self, filenameToFind, path):
        ...
        self._client.ui.showErrorMessage(
            getMessage("added-file-not-in-media-directory-error").format(
                directoryToFind
            )
        )
