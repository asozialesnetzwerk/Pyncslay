...
from syncplay.vendor.python_mpv_jsonipc.python_mpv_jsonipc import (
    log,
    MPV,
    MPVError,
    MPVProcess,
...
class IINAProcess(MPVProcess):
    ...
    def _start_process(self, ipc_socket, args, env):
        ...
        for _ in range(100):  # Give IINA 10 seconds to start.
            ...
            ):  # iina-cli returns immediately after its start
                log.error(
                    "IINA failed with returncode {0}.".format(self.process.returncode)
                )
                break
        else:
