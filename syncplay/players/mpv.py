...
class MpvPlayer(BasePlayer):
    ...
    def run(client, playerPath, filePath, args):
        try:
            ver = MpvPlayer.RE_VERSION.search(
                subprocess.check_output([playerPath, "--version"]).decode("utf-8")
            )
        except:
            ver = None
        constants.MPV_NEW_VERSION = (
            ver is None or int(ver.group(1)) > 0 or int(ver.group(2)) >= 23
        )
        if not constants.MPV_NEW_VERSION:
            from twisted.internet import reactor

            the_reactor = reactor
            the_reactor.callFromThread(
                client.ui.showErrorMessage,
        ...
        constants.MPV_OSC_VISIBILITY_CHANGE_VERSION = (
            False if ver is None else int(ver.group(1)) > 0 or int(ver.group(2)) >= 28
        )
    ...
    def mpvErrorCheck(self, line):
        if "Error parsing option" in line or "Error parsing commandline option" in line:
            ...
            self.reactor.callFromThread(
                self._client.ui.showErrorMessage, getMessage("mpv-version-error"), True
            )
            self.drop()

        if constants and any(
            errormsg in line for errormsg in constants.MPV_ERROR_MESSAGES_TO_REPEAT
        ):
    ...
    def displayMessage(
        ...
        if not self._client._config["chatOutputEnabled"]:
            messageString = self._sanitizeText(
                message.replace("\\n", "<NEWLINE>")
            ).replace("<NEWLINE>", "\\n")
            self._listener.mpvpipe.show_text(
                messageString, duration, constants.MPLAYER_OSD_LEVEL
            )
            return
        messageString = (
            self._sanitizeText(message.replace("\\n", "<NEWLINE>"))
            .replace("\\\\", constants.MPV_INPUT_BACKSLASH_SUBSTITUTE_CHARACTER)
            .replace("<NEWLINE>", "\\n")
        )
    ...
    def displayChatMessage(self, username, message):
        if not self._client._config["chatOutputEnabled"]:
            ...
            return
        username = self._sanitizeText(
            username.replace("\\", constants.MPV_INPUT_BACKSLASH_SUBSTITUTE_CHARACTER)
        )
        message = self._sanitizeText(
            message.replace("\\", constants.MPV_INPUT_BACKSLASH_SUBSTITUTE_CHARACTER)
        )
        messageString = "<{}> {}".format(username, message)
        self._listener.sendLine(
            ["script-message-to", "syncplayintf", "chat", messageString]
        )
    ...
    def setPaused(self, value):
        if self._paused == value:
            self._client.ui.showDebugMessage(
                "Not sending setPaused to mpv as state is already {}".format(value)
            )
            return
    ...
    def _getProperty(self, property_):
        ...
        self._listener.sendLine(
            ["print_text", '"ANS_{}=${{{}}}"'.format(property_, propertyID)]
        )
    ...
    def getCalculatedPosition(self):
        ...
        if diff > constants.MPV_UNRESPONSIVE_THRESHOLD:
            self.reactor.callFromThread(
                ...
                getMessage("mpv-unresponsive-error").format(int(diff)),
                True,
            )
            self.drop()
    ...
    def _storePosition(self, value):
        ...
        if self._recentlyReset():
            ...
            self._position = 0
        elif self._fileIsLoaded() or (
            value < constants.MPV_NEWFILE_IGNORE_TIME
            and self._fileIsLoaded(ignoreDelay=True)
        ):
            ...
            self._client.ui.showDebugMessage(
                "No file loaded so storing position {} as GlobalPosition ({})".format(
                    value, self._client.getGlobalPosition()
                )
            )
    ...
    def lineReceived(self, line):
        ...
        if (
            "Failed to get value of property" in line
            or "=(unavailable)" in line
            or line == "ANS_filename="
            or line == "ANS_length="
            or line == "ANS_path="
        ):
            if "filename" in line:
        ...
        if name == self.POSITION_QUERY:
            ...
            if value != "Quit":
                ...
                self.reactor.callFromThread(
                    self._client.ui.showErrorMessage, self.quitReason, True
                )
            self.drop()
    ...
    def askForStatus(self):
        ...
        self._client.updatePlayerStatus(
            self._paused if self.fileLoaded else self._client.getGlobalPaused(),
    ...
    def drop(self):
        try:
            ...
            self._client.ui.showDebugMessage(
                "Could not send quit message: {}".format(str(e))
            )
        self._takeLocksDown()
    ...
    def _getPausedAndPosition(self):
        self._listener.sendLine(
            ["script-message-to", "syncplayintf", "get_paused_and_position"]
        )
    ...
    def setPosition(self, value):
        if value < constants.DO_NOT_RESET_POSITION_THRESHOLD and self._recentlyReset():
            self._client.ui.showDebugMessage(
                "Did not seek as recently reset and {} below 'do not reset position' threshold".format(
                    value
                )
            )
            return
    ...
    def openFile(self, filePath, resetPosition=False):
        self._client.ui.showDebugMessage(
            "openFile, resetPosition=={}".format(resetPosition)
        )
        if resetPosition:
            self.lastResetTime = time.time()
            if isURL(filePath):
                self._client.ui.showDebugMessage(
                    "Setting additional lastResetTime due to stream"
                )
                self.lastResetTime += constants.STREAM_ADDITIONAL_IGNORE_TIME
        self._loadFile(filePath)
        if self._paused != self._client.getGlobalPaused():
            self._client.ui.showDebugMessage(
                "Want to set paused to {}".format(self._client.getGlobalPaused())
            )
        else:
            self._client.ui.showDebugMessage(
                "Don't want to set paused to {}".format(self._client.getGlobalPaused())
            )
        if resetPosition == False:
            self._client.ui.showDebugMessage(
                "OpenFile setting position to global position: {}".format(
                    self._client.getGlobalPosition()
                )
            )
    ...
    def _sendMpvOptions(self):
        ...
        options.append(
            "OscVisibilityChangeCompatible={}".format(
                constants.MPV_OSC_VISIBILITY_CHANGE_VERSION
            )
        )
    ...
    def _handleUnknownLine(self, line):
        ...
        if line == "<SyncplayUpdateFile>" or "Playing:" in line:
            self._client.ui.showDebugMessage(
                "Not ready to send due to <SyncplayUpdateFile>"
            )
            ...
            self._client.ui.showDebugMessage(
                "Ready to send due to </SyncplayUpdateFile>"
            )

        elif (
            "Failed" in line
            or "failed" in line
            or "No video or audio streams selected" in line
            or "error" in line
        ):
    ...
    def _setOSDPosition(self):
        if self._client._config["chatMoveOSD"] and (
            self._client._config["chatOutputEnabled"]
            or (
                self._client._config["chatInputEnabled"]
                and self._client._config["chatInputPosition"]
                == constants.INPUT_POSITION_TOP
            )
        ):
            self._setProperty("osd-align-y", "bottom")
            self._setProperty(
                "osd-margin-y", int(self._client._config["chatOSDMargin"])
            )
    ...
    def _onFileUpdate(self):
        ...
        if not (self._recentlyReset()):
            self._client.ui.showDebugMessage(
                "onFileUpdate setting position to global position: {}".format(
                    self._client.getGlobalPosition()
                )
            )
            self.reactor.callFromThread(
        ...
        if self._paused != self._client.getGlobalPaused():
            self._client.ui.showDebugMessage(
                "onFileUpdate setting position to global paused: {}".format(
                    self._client.getGlobalPaused()
                )
            )
    ...
    def _fileIsLoaded(self, ignoreDelay=False):
        ...
        return (
            self.fileLoaded
            and self.lastLoadedTime is not None
            and time.time() > (self.lastLoadedTime + constants.MPV_NEWFILE_IGNORE_TIME)
        )
    ...
    def _create_listener(self, playerPath, filePath, args):
        try:
            self._listener = self.__Listener(
                self, self._playerIPCHandler, playerPath, filePath, args
            )
        except ValueError:
            self._client.ui.showMessage(
                getMessage("mplayer-file-required-notification")
            )
            self._client.ui.showMessage(
                getMessage("mplayer-file-required-notification/example")
            )
            self.drop()
            return
        except AttributeError as e:
    ...
    class __Listener(threading.Thread):
        def __init__(
            self, playerController, playerIPCHandler, playerPath, filePath, args
        ):
            ...
            if isMacOS():
                try:
                    ...
                    ytdl_path = subprocess.check_output(
                        ["which", "youtube-dl"], text=True, env=env
                    ).rstrip("\n")
                    ...
                    if "/usr/bin/env" in ytdl_python:
                        ...
                        python_executable = subprocess.check_output(
                            ["which", python_name], text=True, env=env
                        ).rstrip("\n")
                    else:
                        python_executable = ytdl_python
                    pythonLibs = subprocess.check_output(
                        [python_executable, "-E", "-c", "import sys; print(sys.path)"],
                        text=True,
                        env=dict(),
                    )
            ...
            try:
                ...
                self.quitReason = (
                    getMessage("media-player-error").format(str(e))
                    + " "
                    + getMessage("mpv-failed-advice")
                )
        ...
        def checkForReadinessOverride(self):
            if (
                self.lastNotReadyTime
                and time.time() - self.lastNotReadyTime
                > constants.MPV_MAX_NEWFILE_COOLDOWN_TIME
            ):
                            ...
                            if line_command.startswith(command):
                                for itemID, deletionCandidate in enumerate(
                                    ...
                                    if " ".join(deletionCandidate).startswith(command):
                                        self.__playerController._client.ui.showDebugMessage(
                                            "<mpv> Remove duplicate (supersede): {}".format(
                            ...
                            if line == command:
                                for itemID, deletionCandidate in enumerate(
                                    ...
                                    if deletionCandidate == command:
                                        self.__playerController._client.ui.showDebugMessage(
                                            "<mpv> Remove duplicate (delete both): {}".format(
                                                self.sendQueue[itemID]
                                            )
                                        )
                                        self.__playerController._client.ui.showDebugMessage(
        ...
        def processSendQueue(self):
            while self.sendQueue and self.readyToSend:
                if (
                    self.lastSendTime
                    and time.time() - self.lastSendTime
